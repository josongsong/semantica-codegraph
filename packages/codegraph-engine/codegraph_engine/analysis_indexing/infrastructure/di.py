...
TODO(future): Integrate LayeredIRBuilder for full 9-layer IR support.
  Currently uses _PythonIRGenerator (Layer 1 only) due to initialization complexity.
  LayeredIRBuilder requires project_root at construction time, which would require
  refactoring IndexingOrchestrator's initialization flow.
"""
...
from codegraph_shared.common.factory_helpers import safe_factory_call
from codegraph_shared.common.observability import get_logger
from codegraph_shared.common.types import GraphStoreFactory, PyrightDaemonFactory, RepoMapStoreFactory
...
class IndexingContainer:
    ...
    def __init__(
        ...
        """
        from codegraph_shared.common.factory_helpers import validate_factory
    ...
    # ========================================================================
    # RFC-027: IR Document Store (SOTA Integration)
    # ========================================================================

    @cached_property
    def ir_document_store(self):
        """
        IR Document Store (PostgreSQL).

        RFC-027 통합:
            - ExecuteExecutor가 IR Document 로드에 사용
            - IRStage가 IR Document 저장에 사용

        SOTA 원칙:
            - Real storage (PostgreSQL)
            - Auto-migration
            - Error handling
        """
    ...
    def file_hash_store(self):
        ...
        from codegraph_engine.analysis_indexing.infrastructure.content_hash_checker import RedisHashStore
    ...
    def metadata_store(self):
        ...
        from codegraph_shared.infra.metadata import PostgresIndexingMetadataStore
    ...
    def version_store(self):
        ...
        try:
            ...
            # Fallback: Return None if multi_index not available
            return None

    @cached_property
    def session_repository(self):
        """Session repository for snapshot management."""
        from codegraph_engine.analysis_indexing.infrastructure.repositories.in_memory_session_repository import (
    ...
    def background_scheduler(self):
        ...
        from codegraph_engine.analysis_indexing.infrastructure.background_scheduler import BackgroundScheduler
    ...
    def file_watcher_service(self):
        ...
        from codegraph_engine.analysis_indexing.infrastructure.watcher_service import FileWatcherService
    ...
    def mode_controller(self):
        ...
        from codegraph_engine.analysis_indexing.infrastructure.mode_controller import ModeController
    ...
    def orchestrator(self) -> IndexingOrchestrator:
        ...
        from codegraph_engine.code_foundation.infrastructure.semantic_ir import DefaultSemanticIrBuilder
        from codegraph_engine.repo_structure.infrastructure.models import RepoMapBuildConfig
        from codegraph_engine.repo_structure.infrastructure.pagerank import PageRankEngine
        from codegraph_engine.repo_structure.infrastructure.summarizer import (
            CostController,
            InMemorySummaryCache,
            LLMSummarizer,
        )
        from codegraph_engine.repo_structure.infrastructure.tree import RepoMapTreeBuilder

        # Parser registry
        parser_registry = ParserRegistry()

        # IR builder: LayeredIRBuilder (9-layer SOTA)
        # Will be created lazily by orchestrator with project_root
        ir_builder = None  # Lazy initialization in orchestrator
        ...
        # Initialize Mode System (will be called by orchestrator during index_repository)
        # orchestrator.initialize_mode_system() requires repo_path which is not available here
