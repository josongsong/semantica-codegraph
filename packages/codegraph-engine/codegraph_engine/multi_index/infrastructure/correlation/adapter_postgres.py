...
try:
    from codegraph_engine.analysis_indexing.infrastructure.git_history.cochange import CoChangeAnalyzer

    _COCHANGE_AVAILABLE = True
except ImportError:
    CoChangeAnalyzer = None  # type: ignore
    _COCHANGE_AVAILABLE = False

if TYPE_CHECKING:
    from codegraph_engine.analysis_indexing.infrastructure.git_history.cochange import CoChangeAnalyzer

from codegraph_engine.multi_index.infrastructure.correlation.models import (
    CorrelationEntry,
...
class CorrelationIndex:
    ...
    async def build_cochange_index(
        ...
        # Hexagonal: Check if analysis_indexing is available
        if not _COCHANGE_AVAILABLE or CoChangeAnalyzer is None:
            logger.warning("CoChangeAnalyzer not available - analysis_indexing not installed")
            return 0
