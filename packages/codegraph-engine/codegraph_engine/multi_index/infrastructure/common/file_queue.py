...
class FileIndexTask:
    ...
    priority: int = 0
    _counter: int = field(default=0, compare=False, repr=False)  # For stable heap ordering

    def __lt__(self, other: "FileIndexTask") -> bool:
        """
        우선순위 비교 (높은 값이 우선, heapq는 min-heap이므로 부호 반전).
        _counter로 동일 우선순위 시 FIFO 보장.
        """
        if self.priority != other.priority:
            return self.priority > other.priority  # Higher priority first
        return self._counter < other._counter  # Earlier insertion first (FIFO)
...
class FileIndexingQueue:
    ...
    def __init__(
        ...
        # 큐 (우선순위) - heapq for O(log n) insertion/removal
        self.queue: list[FileIndexTask] = []
        self.queue_lock = asyncio.Lock()
        self._task_counter = 0  # Monotonic counter for stable FIFO ordering
