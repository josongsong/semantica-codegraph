...
class IndexVersionStore:
    ...
    async def create_version(
        ...
        Args:
            ...
            snapshot_id: Snapshot ID (default: "main")

        Returns:
            Created IndexVersion
        """
        # Generate version_id (timestamp-based for SQLite compatibility)
    ...
    async def update_status(
        ...
        Args:
            ...
            repo_id: Repository ID (for fail_version)
            duration_ms: Duration in milliseconds (for complete_version)
        """
        if status == IndexVersionStatus.COMPLETED:
            await self.complete_version(repo_id, version_id, duration_ms)
        elif status == IndexVersionStatus.FAILED:
            await self.fail_version(repo_id, version_id, error_message or "Unknown error")
        else:
            ...
            query = """
                ...
                WHERE version_id = $2
            """
            await self.postgres.execute(query, status.value, version_id)
    ...
    def _row_to_version(self, row: dict) -> IndexVersion:
        ...
        return IndexVersion(
            ...
            indexed_at=row.get("indexed_at") or row.get("created_at"),
