...
- Lexical (Tantivy): Full-text code search
- Vector (Qdrant): Semantic search
- Symbol (Memgraph): Graph-based symbol navigation
- Fuzzy (pg_trgm): Fuzzy identifier matching
- Domain (PostgreSQL): Documentation search
- Runtime (future): Execution trace analysis
- Service: Orchestrates all indexes

Note: Imports are lazy to avoid loading heavy dependencies (qdrant, numpy)
when only lightweight components are needed.
"""

from typing import TYPE_CHECKING

from codegraph_engine.multi_index.infrastructure.common import IndexDocument, IndexDocumentTransformer, SearchHit
...
def __getattr__(name: str):
    ...
    if name in (
        ...
        from codegraph_engine.multi_index.infrastructure import factory
