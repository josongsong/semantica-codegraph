...
if TYPE_CHECKING:
    ...
    from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import (
        CallsEdgeAttrs,
        EdgeAttrsBase,
        ImportsEdgeAttrs,
        InheritsEdgeAttrs,
    ...
    from codegraph_engine.code_foundation.infrastructure.graph.edge_validator import (
        EdgeStatus,
        EdgeValidator,
        StaleEdgeInfo,
    )
...
def __getattr__(name: str):
    ...
    if name == "ImpactResult":
        from codegraph_engine.code_foundation.infrastructure.graph.impact_analyzer import ImpactResult

        return ImpactResult
    if name == "SymbolChange":
        from codegraph_engine.code_foundation.infrastructure.graph.impact_analyzer import SymbolChange

        return SymbolChange
    if name == "ChangeType":
        from codegraph_engine.code_foundation.infrastructure.graph.impact_analyzer import ChangeType

        return ChangeType
    if name == "detect_symbol_changes":
        from codegraph_engine.code_foundation.infrastructure.graph.impact_analyzer import detect_symbol_changes
    ...
    if name == "EdgeStatus":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_validator import EdgeStatus

        return EdgeStatus
    if name == "StaleEdgeInfo":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_validator import StaleEdgeInfo

        return StaleEdgeInfo

    # Edge Attrs
    if name == "EdgeAttrsBase":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import EdgeAttrsBase

        return EdgeAttrsBase
    if name == "CallsEdgeAttrs":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import CallsEdgeAttrs

        return CallsEdgeAttrs
    if name == "ImportsEdgeAttrs":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import ImportsEdgeAttrs

        return ImportsEdgeAttrs
    if name == "InheritsEdgeAttrs":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import InheritsEdgeAttrs

        return InheritsEdgeAttrs
    if name == "create_edge_attrs":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import create_edge_attrs

        return create_edge_attrs
    if name == "parse_edge_attrs":
        from codegraph_engine.code_foundation.infrastructure.graph.edge_attrs import parse_edge_attrs
