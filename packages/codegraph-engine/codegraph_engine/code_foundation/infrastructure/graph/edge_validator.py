...
class EdgeValidator:
    ...
    def mark_stale_edges(
        ...
        # 1. 변경 파일 내 심볼 ID 수집 (O(M) where M = changed files, using path index)
        changed_symbol_ids: set[str] = graph.get_node_ids_by_paths(list(changed_files))
        ...
        # Performance: Use incoming index to find edges targeting changed symbols
        # O(S × k) where S = changed symbols, k = avg incoming edges per symbol
        ...
        for symbol_id in changed_symbol_ids:
            # Use incoming edge index instead of O(E) full scan
            incoming_edge_ids = graph.indexes.incoming.get(symbol_id, [])
            target_node = graph.get_node(symbol_id)
            if not target_node:
                continue
    ...
    def mark_deleted_symbol_edges(
        ...
        # Performance: Use incoming index to find edges targeting deleted symbols
        # O(D × k) where D = deleted symbols, k = avg incoming edges per symbol
        for symbol_id in deleted_symbol_ids:
            incoming_edge_ids = graph.indexes.incoming.get(symbol_id, [])
