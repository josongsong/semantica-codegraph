...
class SemanticIrSnapshot:
    ...
    # Phase 2c: Expression IR (for inter-procedural analysis)
    ...
    def get_stats(self) -> dict[str, int]:
        """
        Get detailed statistics from the semantic IR snapshot.

        Returns:
            Dictionary with counts for each IR component.
        """
        dfg_vars = 0
        dfg_edges = 0
        if self.dfg_snapshot:
            dfg_vars = len(self.dfg_snapshot.variables)
            dfg_edges = len(self.dfg_snapshot.edges) if hasattr(self.dfg_snapshot, "edges") else 0

        return {
            "types": len(self.types),
            "signatures": len(self.signatures),
            "bfg_graphs": len(self.bfg_graphs),
            "bfg_blocks": len(self.bfg_blocks),
            "cfg_graphs": len(self.cfg_graphs),
            "cfg_blocks": len(self.cfg_blocks),
            "cfg_edges": len(self.cfg_edges),
            "expressions": len(self.expressions),
            "dfg_variables": dfg_vars,
            "dfg_edges": dfg_edges,
        }
