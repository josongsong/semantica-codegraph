...
from codegraph_engine.code_foundation.infrastructure.analyzers.type_narrowing_full import (
...
class CallSite:
    ...
    receiver_type: str | None = None  # Type of receiver (for methods)
...
class PreciseCallGraphBuilder:
    ...
    def build_precise_cg(
        ...
        logger.info("precise_call_graph_built", num_edges=len(self.edges), num_files=len(ir_documents))
    ...
    def _analyze_body_with_narrowing(self, body: str, initial_types: dict[str, set[str]]) -> dict[int, TypeState]:
        """
    ...
    def _parse_isinstance(self, line: str) -> tuple[str | None, str | None]:
    ...
    def _parse_none_check(self, line: str) -> str | None:
    ...
    def _get_type_state_at_line(self, type_states: dict[int, TypeState], line: int) -> TypeState:
        """Get type state at specific line"""
        # Find closest line <= target line
        valid_lines = [line_no for line_no in type_states.keys() if line_no <= line]
        if valid_lines:
    ...
    def compare_with_basic_cg(self, basic_edges: set[tuple[str, str]]) -> dict[str, int]:
        ...
        return {
            ...
            "precision_gain": ((len(basic_edges - precise_set) / len(basic_edges) * 100) if basic_edges else 0),
        }
