...
class OccurrenceIndex:
    ...
    # 🚀 SOTA: Pre-computed role values for O(1) bit masking
    _ROLE_VALUES: list[tuple[int, "SymbolRole"]] = None  # type: ignore

    @classmethod
    def _ensure_role_values(cls) -> list[tuple[int, "SymbolRole"]]:
        """Lazy-init role values (avoid enum iteration overhead)."""
        if cls._ROLE_VALUES is None:
            cls._ROLE_VALUES = [(r.value, r) for r in SymbolRole if r != SymbolRole.NONE]
        return cls._ROLE_VALUES
    ...
    def add(self, occurrence: Occurrence) -> None:
        ...
        # 🚀 SOTA: Optimized role indexing with int bit masking (no enum overhead)
        roles_int = occurrence.roles.value if hasattr(occurrence.roles, "value") else int(occurrence.roles)
        for role_value, role_enum in self._ensure_role_values():
            if roles_int & role_value:  # int bitwise (fast) instead of enum.__and__
                self.by_role.setdefault(role_enum, []).append(occ_id)

        # 🚀 SOTA: Use int bit masking for stats (no enum overhead)
        self.total_occurrences += 1
        if roles_int & 1:  # SymbolRole.DEFINITION.value = 1
            self.definitions_count += 1
        if roles_int & 8:  # SymbolRole.READ_ACCESS.value = 8
