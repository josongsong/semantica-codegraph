...
class PyrightAdapter:
    ...
    def __init__(
        self,
        project_root: Path,
        shared_client: PyrightLSPClient | None = None,
    ):
        """
        ...
        Args:
            ...
            shared_client: Optional shared PyrightLSPClient (for Layer 3/5 reuse)
                           If provided, avoids creating a new Pyright process
        """
        self.project_root = project_root
        # SOTA: Use shared client if provided (avoids double Pyright process)
        self.client = shared_client if shared_client else PyrightLSPClient(project_root)
