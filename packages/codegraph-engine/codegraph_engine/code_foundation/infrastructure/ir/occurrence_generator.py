...
if TYPE_CHECKING:
    from codegraph_engine.code_foundation.infrastructure.ir.models.core import Edge, Node
...
class OccurrenceGenerator:
    ...
    def _calculate_importance_scores(
        ...
        # 🚀 SOTA: Pre-compute role values for int bitmasking (avoid enum overhead)
        READ_ACCESS_VAL = SymbolRole.READ_ACCESS.value  # 8
        WRITE_ACCESS_VAL = SymbolRole.WRITE_ACCESS.value  # 4
        TEST_VAL = SymbolRole.TEST.value  # 32
        GENERATED_VAL = SymbolRole.GENERATED.value  # 16

        # Count references per symbol (using int bit operations)
        ref_counts: dict[str, int] = {}
        for occ in occurrences:
            roles_int = occ.roles.value if hasattr(occ.roles, "value") else int(occ.roles)
            if roles_int & (READ_ACCESS_VAL | WRITE_ACCESS_VAL):  # is_reference or is_write
