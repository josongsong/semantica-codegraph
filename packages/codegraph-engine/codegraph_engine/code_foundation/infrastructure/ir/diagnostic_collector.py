...
if TYPE_CHECKING:
    from codegraph_engine.code_foundation.infrastructure.ir.lsp.adapter import LSPDiagnostic, MultiLSPManager
...
class DiagnosticCollector:
    ...
    async def collect(self, ir_docs: dict[str, "IRDocument"]) -> DiagnosticIndex:
        ...
        for language, files in files_by_lang.items():
            # Use async version to ensure adapter is initialized
            client = await self.lsp_manager.get_client_async(language)
            if not client:
