...
from codegraph_engine.code_foundation.infrastructure.ir.models.core import Edge, Node, NodeKind
...
class IncrementalIRBuilder:
    ...
    def _compute_content_hash(node: Node) -> str:
        ...
        elif node.kind == NodeKind.METHOD_REFERENCE:
            content_parts.append(node.attrs.get("ref_type", ""))
            content_parts.append(node.attrs.get("target", ""))

        elif node.kind == NodeKind.LAMBDA and node.attrs.get("is_anonymous_class"):
    ...
    def compute_delta(
        ...
        for old_id in removed_node_ids:
            ...
            if old_node.kind not in (NodeKind.LAMBDA, NodeKind.METHOD_REFERENCE):
                continue
