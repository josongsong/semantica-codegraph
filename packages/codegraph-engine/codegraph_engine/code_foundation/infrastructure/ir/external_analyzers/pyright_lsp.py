...
from codegraph_engine.code_foundation.infrastructure.ir.external_analyzers.base import TypeInfo
...
class PyrightLSPClient:
    ...
    def __init__(self, project_root: Path | str):
        ...
        self._request_id = 0
        # Serialize JSON-RPC writes (requests + notifications) to avoid interleaving across threads
        # ExpressionBuilder runs many hover() calls in parallel.
        self._io_lock = threading.Lock()

        # Response queue with LRU eviction (FIX: Critical #1 - Memory leak)
        self._responses = LRUResponseQueue(max_size=100)

        # Opened documents with versions: uri -> (version, content_hash)
        self._opened_documents: set[str] = set()
        self._document_versions: dict[str, tuple[int, str]] = {}  # uri -> (version, content_hash)
        ...
        # Key: (file_path, content_hash, line, col) - content_hash ensures cache invalidation on file change
        self._hover_cache: dict[tuple[str, str, int, int], dict[str, Any]] = {}
        self._hover_cache_lock = threading.Lock()
        # Content hash cache to avoid re-reading files: file_path -> (mtime, content_hash)
        self._content_hash_cache: dict[str, tuple[float, str]] = {}
    ...
    def _start_server(self):
        ...
        try:
            ...
            # Use shutil.which first, then fallback to common locations
            import shutil
            ...
            if not cmd:
                raise FileNotFoundError("pyright-langserver not found in PATH or common locations")
    ...
    def _send_request(self, method: str, params: dict[str, Any], timeout: float = 10.0) -> dict[str, Any] | None:
        ...
        # Thread-safe send: request_id allocation + stdin write must be atomic
        with self._io_lock:
    ...
    def _send_notification(self, method: str, params: dict[str, Any]):
        ...
        # Thread-safe write (notifications can interleave with requests otherwise)
        with self._io_lock:
            try:
                self._server_process.stdin.write(message)
                self._server_process.stdin.flush()
    ...
    def _ensure_document_opened(self, file_path: Path):
        """
        Ensure document is opened in LSP server with current content.

        If document was previously opened but content changed, sends didChange.
        This ensures Pyright always has the latest content for accurate analysis.
        """
        # Convert to absolute path to ensure as_uri() works
        ...
        if uri in self._opened_documents:
            ...
            old_version, old_hash = self._document_versions.get(uri, (1, ""))
            ...
            self._document_versions[uri] = (new_version, content_hash)
            ...
            self._send_notification("textDocument/didChange", change_params)
            logger.debug(f"[Pyright] Document changed: {uri} (v{old_version} -> v{new_version})")

            # Wait for re-analysis
            self._wait_for_analysis_complete(uri, timeout=5.0)
            return

        # First time opening - send didOpen notification
        params = {
            "textDocument": {
                "uri": uri,
                "languageId": "python",
        ...
        self._document_versions[uri] = (1, content_hash)
    ...
    def _get_content_hash(self, file_path: Path) -> str:
        """
        Get content hash for file, using mtime-based cache.

        Uses file modification time to avoid re-reading unchanged files.
        Returns MD5 hash of file content (first 16 chars).
        ...
        try:
            ...
            with self._content_hash_lock:
                cached = self._content_hash_cache.get(file_key)
            ...
            with self._content_hash_lock:
                self._content_hash_cache[file_key] = (mtime, content_hash)
    ...
    def hover(self, file_path: Path | str, line: int, col: int) -> dict[str, Any] | None:
        """
        Get hover information (type + docs) at position.

        Args:
            file_path: Source file path (Path or str)
        ...
        # Normalize path - handle str input from BatchLSPFetcher
        if isinstance(file_path, str):
            file_path = Path(file_path)
        # Convert to absolute path for LSP URI
        abs_path = file_path if file_path.is_absolute() else file_path.resolve()

        # Get content hash for cache key (ensures cache invalidation on file change)
        content_hash = self._get_content_hash(abs_path)

        # Check cache with content hash (FIX: cache invalidation on file change)
        cache_key = (str(abs_path), content_hash, line, col)
        with self._hover_cache_lock:
            if cache_key in self._hover_cache:
                return self._hover_cache[cache_key]

        # Ensure document is opened
        self._ensure_document_opened(abs_path)
    ...
    def definition(self, file_path: Path | str, line: int, col: int) -> BaseLocation | None:
        """
    ...
    def references(self, file_path: Path | str, line: int, col: int) -> list[BaseLocation]:
        """
