...
class PyrightExternalAnalyzer:
    ...
    def __init__(
        self,
        project_root: Path,
        cache: "PyrightResultCache | None" = None,
        shared_lsp_client: "PyrightLSPClient | None" = None,
    ):
        """
        ...
        Args:
            ...
            cache: Optional shared cache for Layer 3/5 result sharing
            shared_lsp_client: Optional shared PyrightLSPClient (for Layer 3/5 reuse)
        ...
        if shared_lsp_client:
            self._lsp_client = shared_lsp_client
            self._daemon = PyrightSemanticDaemon(project_root, shared_lsp_client=shared_lsp_client)
            ...
            self._lsp_client = self._daemon._lsp_client
            self._owns_client = True

        self.project_root = project_root

        # SOTA: Shared cache for Layer 3/5 result sharing
        self._cache = cache
        self._file_contents: dict[str, str] = {}  # file_path -> content for cache key
    ...
    def open_file(self, file_path: Path, content: str) -> None:
        ...
        # Store content for cache key generation
        self._file_contents[str(file_path)] = content
    ...
    def open_files(self, files: list[tuple[Path, str]]) -> None:
        ...
        # Store contents for cache key generation
    ...
    def hover(self, file_path: Path | str, line: int, col: int) -> dict | None:
        """
        Get hover information (type + docs) at position.

        SOTA: Cache-first lookup, then Pyright call on miss.
        ...
        content = self._file_contents.get(file_key)

        # SOTA: Check cache first
        if self._cache is not None:
            cached = self._cache.get_hover(file_path, content, line, col)
            if cached is not None:
                return cached.to_dict()

        # Cache miss - call Pyright
        result = self._lsp_client.hover(file_path, line, col)

        # Store in cache for Layer 5 reuse
        if self._cache is not None and content is not None:
            self._cache.put_hover(file_path, content, line, col, result)

        return result

    def definition(self, file_path: Path | str, line: int, col: int) -> dict | None:
        """
        ...
        # SOTA: Check cache first
        if self._cache is not None:
            cached = self._cache.get_definition(file_path, content, line, col)
            if cached is not None:
                return cached.to_dict()

        # Cache miss - call Pyright
        result = self._lsp_client.definition(file_path, line, col)

        # Store in cache for Layer 5 reuse
        if self._cache is not None and content is not None:
            self._cache.put_definition(file_path, content, line, col, result)
