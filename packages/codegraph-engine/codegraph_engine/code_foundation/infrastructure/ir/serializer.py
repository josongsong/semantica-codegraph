...
class IRSerializer:
    ...
    def validate_roundtrip(cls, ir_doc: IRDocument) -> tuple[bool, list[str]]:
        ...
        if len(ir_doc.nodes) != len(ir_doc_recovered.nodes):
            ...
            for i, (orig, recovered) in enumerate(zip(ir_doc.nodes, ir_doc_recovered.nodes, strict=False)):
        ...
        if len(ir_doc.edges) != len(ir_doc_recovered.edges):
            ...
            for i, (orig, recovered) in enumerate(zip(ir_doc.edges, ir_doc_recovered.edges, strict=False)):
                if orig.id != recovered.id:
