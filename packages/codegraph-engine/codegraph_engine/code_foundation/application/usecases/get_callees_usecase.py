"""
GetCallees UseCase (Primitive Tool)

RFC-052: MCP Service Layer Architecture
Get functions that are called by a given symbol.
...
MCP Tool Mapping:
- MCP: get_callees(symbol, limit)
- UseCase: GetCalleesUseCase.execute(GetCalleesRequest)
...
@dataclass
class GetCalleesRequest(UseCaseRequest):
    """GetCallees request"""
...
@dataclass
class GetCalleesResponse(UseCaseResponse):
    """GetCallees response"""

    callees: list[CalleeInfo] | None = None


class GetCalleesUseCase(BaseUseCase[GetCalleesRequest, GetCalleesResponse]):
    """
    GetCallees Primitive UseCase.
    ...
    async def _execute_impl(
        ...
        try:
            ...
            # Query callees via Port interface
            callees_from_port = await call_graph.get_callees(
            ...
            # Convert PortCalleeInfo to local CalleeInfo
            callees = [
                CalleeInfo(
                ...
                for c in callees_from_port
            ]

            verification = self._create_verification_snapshot(snapshot_id, self._create_dummy_plan())
            ...
            logger.error("get_callees_failed", error=str(e), exc_info=True)
            error = AnalysisError(
                error_code=ErrorCode.INTERNAL_ERROR,
    ...
    def _get_session_id(self, request: GetCalleesRequest) -> str:
        return request.session_id or f"session_{uuid.uuid4().hex[:8]}"

    def _get_repo_id(self, request: GetCalleesRequest) -> str:
        return request.repo_id

    def _create_error_response(self, error: str) -> GetCalleesResponse:
        return GetCalleesResponse(
            verification=self._create_error_verification(),
            error=AnalysisError(
                error_code=ErrorCode.INTERNAL_ERROR,
