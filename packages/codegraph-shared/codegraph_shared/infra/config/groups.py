...
class VectorConfig(BaseModel):
    """Qdrant 벡터 검색 설정 (타입 안전성 강화)."""

    # 모드 선택 (Literal로 타입 제약)
    mode: str = Field(
        default="embedded",
        description="실행 모드: memory (테스트) | embedded (로컬) | server (프로덕션)",
        pattern="^(memory|embedded|server)$",
    )
    storage_path: str = Field(
        default="./data/qdrant_storage",
        description="embedded 모드 저장 경로",
        min_length=1,
    )

    # Server 모드용
    url: str = Field(default="http://localhost:6333", description="Qdrant URL")
    host: str = Field(default="localhost", min_length=1, description="Qdrant 호스트")
    ...
    timeout: int = Field(default=60, ge=1, le=600, description="연결 타임아웃 (초)")

    # 공통
    collection_name: str = Field(default="codegraph", min_length=1, max_length=255, description="컬렉션 이름")
    vector_size: int = Field(default=1024, ge=1, le=65536, description="벡터 차원 (bge-m3)")
    upsert_concurrency: int = Field(default=4, ge=1, le=16, description="동시 upsert 배치 수")

    # Embedded 모드 안전성
    check_disk_space: bool = Field(default=True, description="디스크 공간 체크 (embedded)")
    min_disk_space_mb: int = Field(default=100, ge=10, le=10000, description="최소 디스크 공간 (MB)")


class LexicalConfig(BaseModel):
    """렉시컬 검색 설정 (Tantivy)."""

    tantivy_index_path: str = Field(default="./data/tantivy_index", description="Tantivy 인덱스 디렉토리")
    tantivy_heap_size_mb: int = Field(default=512, ge=128, le=4096, description="Tantivy writer heap size (MB)")
    tantivy_num_threads: int = Field(default=4, ge=1, le=16, description="Tantivy indexing threads")
...
class TypeInferenceConfig(BaseModel):
    """
    Type Inference 설정 (RFC-030).

    Self-contained type inference with optional Pyright fallback.
    """

    # Pyright fallback
    enable_pyright_fallback: bool = Field(
        default=True,
        description="Pyright LSP fallback 활성화 (자체 추론 실패 시)",
    )

    # Self-contained inference options
    enable_literal_inference: bool = Field(
        default=True,
        description="리터럴 타입 추론 활성화",
    )
    enable_call_graph_inference: bool = Field(
        default=True,
        description="Call graph 기반 반환 타입 추론 활성화",
    )
    enable_builtin_method_inference: bool = Field(
        default=True,
        description="빌트인 메서드 타입 테이블 활성화",
    )

    # Builtin methods config
    builtin_methods_path: str | None = Field(
        default=None,
        description="커스텀 빌트인 메서드 YAML 경로 (기본값 사용 시 None)",
    )
