...
class QueryConfig:
    ...
    MIN_MAX_VULNERABILITIES: int = 1
    MAX_MAX_VULNERABILITIES: int = 100000
    MIN_TIMEOUT_MS: int = 1000  # 1 second
    MAX_TIMEOUT_MS: int = 3600000  # 1 hour
    ...
    def _validate(self) -> None:
        ...
        # Validate max_vulnerabilities
        if not (self.MIN_MAX_VULNERABILITIES <= self.max_vulnerabilities <= self.MAX_MAX_VULNERABILITIES):
            raise ValueError(
                f"max_vulnerabilities must be between {self.MIN_MAX_VULNERABILITIES} "
                f"and {self.MAX_MAX_VULNERABILITIES}, got {self.max_vulnerabilities}"
            )

        # Validate timeout_ms
        if not (self.MIN_TIMEOUT_MS <= self.timeout_ms <= self.MAX_TIMEOUT_MS):
            raise ValueError(
                f"timeout_ms must be between {self.MIN_TIMEOUT_MS} and {self.MAX_TIMEOUT_MS}, got {self.timeout_ms}"
            )

        # Validate enabled/disabled rules don't overlap
        if self.enabled_rules:
            overlap = set(self.enabled_rules) & set(self.disabled_rules)
            if overlap:
                raise ValueError(f"Rules cannot be both enabled and disabled: {overlap}")
...
class QueryEngine:
    ...
    def _filter_by_severity(
        ...
        Uses Severity enum's __lt__ comparison (proper ordering).
        ...
        Returns:
            Filtered vulnerabilities (min_severity or higher)
        """
        # Use proper enum comparison via __ge__ (implemented via __lt__)
        # min_severity=HIGH means keep HIGH and CRITICAL
        return [v for v in vulnerabilities if v.severity >= self.config.min_severity]
...
def create_query_engine(
    ...
    Example:
        ...
        # Only specific rules (example)
        engine = create_query_engine(
            enabled_rules=["specific-rule-id"],
