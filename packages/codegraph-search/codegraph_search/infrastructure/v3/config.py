...
class QueryEnhancementConfig:
    """Query enhancement configuration (HyDE, RAG-Fusion)."""

    # HyDE (Hypothetical Document Embeddings)
    enable_hyde: bool = False
    hyde_complexity_threshold: float = 0.7
    hyde_num_hypothetical_docs: int = 3

    # RAG-Fusion (Multi-query generation)
    enable_rag_fusion: bool = False
    rag_fusion_num_queries: int = 3
    rag_fusion_min_query_length: int = 10
...
class PositionBiasConfig:
    """Position bias mitigation configuration."""

    enabled: bool = True
    min_chunks_for_reordering: int = 5
    strategy: str = "lost_in_middle"  # or "balanced"


@dataclass
class GraphExpansionConfig:
    """Graph expansion configuration for flow queries."""

    max_forward_seeds: int = 10  # Max seeds for callees
    max_backward_seeds: int = 5  # Max seeds for callers
    max_depth: int = 3
...
class SearchLimitsConfig:
    ...
    default_limit: int = 40  # Default max results after fusion
    default_symbol_limit: int = 20  # Default max symbol results
    log_query_preview_length: int = 50  # Query preview in logs
    log_max_results: int = 20  # Max results to log
...
class RetrieverV3Config:
    ...
    def __post_init__(self):
        ...
        if self.query_enhancement is None:
            self.query_enhancement = QueryEnhancementConfig()
        if self.cross_encoder is None:
            self.cross_encoder = CrossEncoderConfig()
        if self.position_bias is None:
            self.position_bias = PositionBiasConfig()
        if self.graph_expansion is None:
            self.graph_expansion = GraphExpansionConfig()
        if self.search_limits is None:
            self.search_limits = SearchLimitsConfig()
    ...
    def from_settings(cls, settings=None):
        ...
        """
        if settings is None:
            from codegraph_shared.infra.config.settings import settings as default_settings
