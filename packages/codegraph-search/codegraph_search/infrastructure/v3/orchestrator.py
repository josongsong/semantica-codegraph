...
Performance (Benchmarked on typer dataset, M4 Max, 2025-12-20):
    E2E Pipeline (Full):
    - Mean:   29.7ms (10 queries)
    - P50:    30.4ms
    - P95:    41.9ms
    - Best:   12.3ms

    Breakdown (estimated from metrics):
    - Multi-index search: ~3-4ms (parallel) / ~9ms (sequential)
    - V3 Fusion: ~1ms
    - Cross-encoder (when enabled): ~5-10ms
    - Observability/Logging: ~2-5ms
    - Other overhead: ~10-15ms
...
from .types import FusionStrategy, IntentType, SearchMode, SourceType
...
if TYPE_CHECKING:
    ...
    from codegraph_search.infrastructure.graph.cost_aware_expander import CostAwareGraphExpander
...
class RetrieverV3Orchestrator:
    ...
    async def search(
        ...
        Performance (Benchmarked E2E, typer dataset, 2025-12-20):
            - E2E Mean: 29.7ms (P50: 30.4ms, Best: 12.3ms)
            - Multi-index search: ~3-4ms (parallel) / ~9ms (sequential)
            - V3 Fusion: ~1ms
            - Cross-encoder (when enabled): ~5-10ms
            - Total overhead (logging, metrics): ~15-20ms
            - Routing optimization: -30% for symbol/concept queries
        """
        # Apply default limits from config
        if limit is None:
            limit = self.v3_service.config.search_limits.default_limit
        if symbol_limit is None:
            symbol_limit = self.v3_service.config.search_limits.default_symbol_limit
        ...
        if self.self_rag:
            ...
            if not should_retrieve:
                ...
                preview_len = self.v3_service.config.search_limits.log_query_preview_length
                logger.info("self_rag_skipped_retrieval", query=query[:preview_len])
                return [], IntentProbability({}, IntentType.GENERAL.value), {"self_rag_skipped": True}, {}
        ...
        if self.hyde_processor and self.v3_service.config.query_enhancement.enable_hyde:
            hyde_result = await self.hyde_processor.process_query(
                query,
                query_complexity=self.v3_service.config.query_enhancement.hyde_complexity_threshold,
            )
        ...
        if self.multi_query and self.v3_service.config.query_enhancement.enable_rag_fusion:
        ...
        # ============================================================
        if dominant_intent == IntentType.FLOW.value and self.graph_expander:
            graph_hits = await self._expand_graph_for_flow(repo_id, snapshot_id, query, hits_by_strategy, intent_prob)
            if graph_hits:
                hits_by_strategy[SourceType.GRAPH.value] = graph_hits
        ...
        if (
            self.position_reorderer
            and self.v3_service.config.position_bias.enabled
            and len(fused_results) >= self.v3_service.config.position_bias.min_chunks_for_reordering
        ):
            from codegraph_search.infrastructure.context_builder.position_bias import RankedChunk
        ...
        logger.info(
            "search_complete",
            query_preview=query[: self.v3_service.config.search_limits.log_query_preview_length],
        ...
        if self.search_logger:
            try:
                ...
                max_log_results = self.v3_service.config.search_limits.log_max_results
                results_for_logging = [
                    {
                    ...
                    for idx, result in enumerate(fused_results[:max_log_results])
                ]
                ...
                await self.search_logger.log_search(
                    ...
                    fusion_strategy=FusionStrategy.RRF_V3.value,
    ...
    async def _search_with_routing(
        ...
        """
        from codegraph_search.infrastructure.routing.strategy_router import StrategyType
    ...
    async def _expand_graph_for_flow(
        ...
        try:
            ...
            seed_symbol_ids = []
            for strategy in [SourceType.SYMBOL.value, SourceType.LEXICAL.value]:
            ...
            max_forward = self.v3_service.config.graph_expansion.max_forward_seeds
            graph_hits = await self.graph_expander.expand_flow(
                start_symbol_ids=seed_symbol_ids[:max_forward],
                direction=TraversalDirection.FORWARD,
                intent=IntentType.FLOW.value,
            )

            # Also expand backward for hub discovery
            max_backward = self.v3_service.config.graph_expansion.max_backward_seeds
            backward_hits = await self.graph_expander.expand_flow(
                start_symbol_ids=seed_symbol_ids[:max_backward],
    ...
    def record_feedback(
        ...
        try:
            from codegraph_search.infrastructure.adaptive.weight_learner import FeedbackSignal
    ...
    async def _search_sequential(
        ...
        if self.symbol_index:
            t = time.perf_counter()
            hits_by_strategy[SourceType.SYMBOL.value] = await self._search_symbol(
                repo_id, snapshot_id, query, symbol_limit
            )
            ...
            record_histogram("retriever_strategy_results_count", len(hits_by_strategy[SourceType.SYMBOL.value]))

        if self.vector_index:
            t = time.perf_counter()
            hits_by_strategy[SourceType.vector.name.value] = await self._search_vector(
                repo_id, snapshot_id, query, limit
            )
            duration_ms = (time.perf_counter() - t) * 1000
            timings["vector_ms"] = duration_ms
            record_histogram("retriever_strategy_duration_ms", duration_ms)
            record_histogram("retriever_strategy_results_count", len(hits_by_strategy[SourceType.vector.name.value]))

        if self.lexical_index:
            t = time.perf_counter()
            hits_by_strategy[SourceType.lexical.name.value] = await self._search_lexical(
                repo_id, snapshot_id, query, limit
            )
            duration_ms = (time.perf_counter() - t) * 1000
            timings["lexical_ms"] = duration_ms
            record_histogram("retriever_strategy_duration_ms", duration_ms)
            record_histogram("retriever_strategy_results_count", len(hits_by_strategy[SourceType.lexical.name.value]))

        if self.graph_index:
            t = time.perf_counter()
            hits_by_strategy[SourceType.graph.name.value] = await self._search_graph(repo_id, snapshot_id, query, limit)
            duration_ms = (time.perf_counter() - t) * 1000
            timings["graph_ms"] = duration_ms
            record_histogram("retriever_strategy_duration_ms", duration_ms)
            record_histogram("retriever_strategy_results_count", len(hits_by_strategy[SourceType.graph.name.value]))
    ...
    async def _search_symbol(self, repo_id: str, snapshot_id: str, query: str, limit: int) -> list[SearchHit]:
        ...
        try:
            ...
            record_counter(
                "retriever_strategy_errors_total", labels={"strategy": SourceType.SYMBOL.value, "mode": "individual"}
            )
            return []
    ...
    async def _search_vector(self, repo_id: str, snapshot_id: str, query: str, limit: int) -> list[SearchHit]:
        ...
        try:
            ...
            record_counter(
                "retriever_strategy_errors_total",
                labels={"strategy": SourceType.vector.name.value, "mode": "individual"},
            )
            return []
    ...
    async def _search_lexical(self, repo_id: str, snapshot_id: str, query: str, limit: int) -> list[SearchHit]:
        ...
        try:
            ...
            record_counter(
                "retriever_strategy_errors_total",
                labels={"strategy": SourceType.lexical.name.value, "mode": "individual"},
            )
            return []
    ...
    async def _search_graph(self, repo_id: str, snapshot_id: str, query: str, limit: int) -> list[SearchHit]:
        ...
        try:
            ...
            record_counter(
                "retriever_strategy_errors_total",
                labels={"strategy": SourceType.graph.name.value, "mode": "individual"},
            )
            return []
