...
class HardNegativeMiner:
    ...
    def log_user_selection(
        ...
        for _i, result in enumerate(shown_results[:selected_rank], start=1):
        ...
        if len(self.training_data) >= self.retraining_threshold:
            logger.info(f"Retraining threshold reached ({len(self.training_data)} samples) - triggering retraining")
    ...
    def _load_existing_data(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.training_data)} existing training samples from {self.storage_path}")
