...
class DependencyGraphExtractor:
    ...
    def _get_dependency_targets(self, symbol_id: str) -> list[str]:
        ...
        Performance: O(k) where k = outgoing edges from symbol (using index)
        """
        targets: list[str] = []
        ...
        # Fall back to GraphDocument - use outgoing index for O(k) instead of O(E)
        elif self.graph_doc:
            edge_ids = self.graph_doc.indexes.outgoing.get(symbol_id, [])
            for edge_id in edge_ids:
                edge = self.graph_doc.edge_by_id.get(edge_id)
                if edge and edge.kind.value in self.DEPENDENCY_EDGES:
