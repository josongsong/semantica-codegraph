...
class ChunkTrimmer:
    ...
    def trim(self, content: str, current_tokens: int) -> tuple[str, int, str]:
        ...
        # Estimate new token count (~0.75 tokens per word)
        words = len(trimmed_content.split())
        new_token_count = max(1, int(words * 0.75)) if words else len(trimmed_content) // 3
        ...
        logger.debug(
            f"Trimmed chunk: {current_tokens} → {new_token_count} tokens (saved {current_tokens - new_token_count})"
        )
