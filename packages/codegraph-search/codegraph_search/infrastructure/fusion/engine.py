...
class FusionEngine:
    ...
    Phase 1 Enhancement: Git History Integration
    - Enriches hits with git metrics (churn, recency, ownership)
    - Reranks based on intent-aware git boosting
    """
    ...
    def __init__(
        ...
        Args:
            ...
            repo_path: Repository path for git enrichment (optional)
            enable_git_enrichment: Enable git history enrichment and ranking
        """
        ...
        self.git_ranker = None
        if enable_git_enrichment and repo_path:
            try:
                ...
                self.git_adapter = GitHistoryAdapter(repo_path)
                self.git_ranker = GitAwareRanker()
                logger.info("Git enrichment enabled for FusionEngine")
            except Exception as e:
                logger.warning(f"Failed to initialize git enrichment: {e}")
        else:
            logger.debug("Git enrichment disabled")
    ...
    def fuse(
        ...
        normalized_results = {
            ...
            "docstring": self.normalizer.normalize_hits(multi_result.docstring_hits, "docstring"),
        }
        ...
        # Phase 1: Git Enrichment (P0-1)
        if self.git_adapter:
            fused_hits = self.git_adapter.enrich_fused_hits(fused_hits)

        # Phase 2: Git-aware Reranking (P0-1)
        if self.git_ranker:
            fused_hits = self.git_ranker.rerank(fused_hits, intent)
        else:
            # Fallback: sort by priority score only
            fused_hits.sort(key=lambda h: h.priority_score, reverse=True)

        avg_score = sum(h.fused_score for h in fused_hits) / len(fused_hits) if fused_hits else 0.0
        logger.info(
            f"Fusion complete: {len(chunk_hits)} unique chunks, "
            f"intent={intent.kind.value}, "
            f"avg_fused_score={avg_score:.3f}, "
            f"git_enriched={self.git_adapter is not None}"
        )
