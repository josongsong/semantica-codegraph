...
from codegraph_search.infrastructure.hybrid.reranker import CrossEncoderPort, RerankedChunk

if TYPE_CHECKING:
    from codegraph_search.infrastructure.hybrid.cross_encoder_reranker import CrossEncoderReranker
    from codegraph_search.infrastructure.hybrid.late_interaction import (
        EmbeddingModelPort,
...
def __getattr__(name: str):
    ...
    if name == "LateInteractionSearch":
        from codegraph_search.infrastructure.hybrid.late_interaction import LateInteractionSearch

        return LateInteractionSearch
    if name == "ScoredChunk":
        from codegraph_search.infrastructure.hybrid.late_interaction import ScoredChunk

        return ScoredChunk
    if name == "EmbeddingModelPort":
        from codegraph_search.infrastructure.hybrid.late_interaction import EmbeddingModelPort
