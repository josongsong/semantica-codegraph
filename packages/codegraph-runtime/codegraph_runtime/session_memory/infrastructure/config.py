...
# Context variable for async/request-scoped config overrides
_config_context: ContextVar[MemorySystemConfig | None] = ContextVar("memory_config", default=None)
...
def get_config() -> MemorySystemConfig:
    ...
    Priority:
    1. Context-scoped config (for async overrides)
    2. Global singleton
    3. Load from environment

    Thread-safe with double-checked locking.
    """
    # Check context-scoped config first (for request overrides)
    ctx_config = _config_context.get()
    if ctx_config is not None:
        return ctx_config

    # Double-checked locking for global singleton
    global _config
    if _config is None:
        with _config_lock:
            if _config is None:
...
def set_config(config: MemorySystemConfig) -> None:
    ...
    For production, use set_context_config() for request-scoped overrides.
    """
    global _config
    with _config_lock:
        _config = config
...
def set_context_config(config: MemorySystemConfig | None) -> None:
    """
    Set context-scoped configuration.

    Use this for async/request-scoped config overrides.
    The config will only apply to the current async context.

    Args:
        config: Config to use, or None to clear override
    """
    _config_context.set(config)


def reset_config() -> None:
    """Reset both global and context config."""
    global _config
    with _config_lock:
        _config = None
