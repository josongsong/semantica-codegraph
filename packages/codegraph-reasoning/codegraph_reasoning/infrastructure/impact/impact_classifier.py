...
from codegraph_reasoning.domain.models import (
...
class ImpactClassifier:
    ...
    def classify(self, old_hash: SymbolHash, new_hash: SymbolHash) -> ImpactType:
        ...
        # 4. 변경 없음
        return ImpactType(
            level=HashBasedImpactLevel.NO_IMPACT, affected_symbols=[], reason="No change detected", confidence=1.0
        )
    ...
    def get_changed_symbols(self, impact_types: dict[str, ImpactType]) -> set[str]:
        ...
        for symbol_id, impact in impact_types.items():
            if impact.level != HashBasedImpactLevel.NO_IMPACT:
    ...
    def get_signature_changed_symbols(self, impact_types: dict[str, ImpactType]) -> set[str]:
        ...
        for symbol_id, impact in impact_types.items():
            if impact.level == HashBasedImpactLevel.SIGNATURE_CHANGE:
                changed.add(symbol_id)
    ...
    def summary(self, impact_types: dict[str, ImpactType]) -> dict[str, int]:
        ...
        for impact in impact_types.values():
            if impact.level == HashBasedImpactLevel.NO_IMPACT:
                summary["no_impact"] += 1
            elif impact.level == HashBasedImpactLevel.IR_LOCAL:
                summary["ir_local"] += 1
            elif impact.level == HashBasedImpactLevel.SIGNATURE_CHANGE:
                summary["signature_change"] += 1
            elif impact.level == HashBasedImpactLevel.STRUCTURAL_CHANGE:
