...
pub struct Node {
    ...
    /// Expression ID for branch condition (Condition/Loop nodes)
    /// Links to ExpressionIR for SMT solver to analyze path feasibility
    /// Example: `if x > 10` → condition_expr_id points to Compare(Gt, x, 10)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub condition_expr_id: Option<usize>,

    /// Condition expression as string (fallback when ExpressionIR unavailable)
    /// Example: "x > 10", "is_admin", "len(items) > 0"
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Node {
    ...
    /// Set condition expression ID (for Condition/Loop nodes)
    /// Links to ExpressionIR for SMT path feasibility analysis
    pub fn with_condition_expr_id(mut self, expr_id: usize) -> Self {
        self.condition_expr_id = Some(expr_id);
        self
    }

    /// Set condition text (fallback when ExpressionIR unavailable)
    ...
    /// Alias for with_parent (for API consistency)
    pub fn with_parent_id(self, parent_id: impl Into<String>) -> Self {
        self.with_parent(parent_id)
    }

    /// Set is_async flag for async functions/methods
...
pub struct NodeBuilder {
    ...
    condition_expr_id: Option<usize>,
...
impl NodeBuilder {
    ...
    /// Set condition text for Condition/Loop nodes
    /// Fallback when ExpressionIR is unavailable
    pub fn with_condition_text(mut self, text: impl Into<String>) -> Self {
        self.condition_text = Some(text.into());
        self
    }
    ...
    pub fn build(self) -> Result<Node, String> {
        ...
        Ok(Node {
            ...
            condition_expr_id: self.condition_expr_id,
...
mod tests {
    ...
    fn test_builder_validation_non_callable_with_parameters() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("should not have parameters"));
    }
    ...
    fn test_builder_validation_non_callable_with_return_type() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("should not have return_type"));
    }
    ...
    fn test_builder_validation_non_class_with_base_classes() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("should not have base_classes"));
    }
