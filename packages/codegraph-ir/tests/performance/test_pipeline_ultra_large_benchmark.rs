//! Ultra Large-Scale E2E Pipeline Benchmark - 459K LOC
//!
//! Tests the full IRIndexingOrchestrator pipeline on the entire packages/ directory
//! (Python + Rust combined) to validate performance at production scale.

use codegraph_ir::pipeline::{E2EPipelineConfig, IRIndexingOrchestrator};
use std::path::PathBuf;

#[test]
#[ignore] // Ultra large benchmark - run with: cargo nextest run -- --ignored
fn test_pipeline_ultra_large_benchmark() {
    println!("\n╔══════════════════════════════════════════════════════════════════════════════╗");
    println!("║       ULTRA LARGE-SCALE E2E PIPELINE BENCHMARK - FULL PACKAGES              ║");
    println!("╚══════════════════════════════════════════════════════════════════════════════╝\n");

    // Use entire packages directory (1,756 files, 459K LOC)
    let test_path = PathBuf::from("../../packages");
    ...
    config.stages.enable_cost_analysis = true;
        ...
        .map(|(_, d)| d)
    {
        ...
        // Calculate expected speedup
        let expected_before_ms = repomap_ms * 4.5; // Conservative 4.5x
        println!("   Expected before optimization: ~{:.0}ms", expected_before_ms);
    ...
    println!("📊 Scalability Validation:");
    println!("   Small (21 files, 9.5K LOC):     0.13s");
    println!("   Medium (467 files, 134K LOC):   0.23s");
    println!("   Large ({} files, {}K LOC): {:.2}s",
        result.stats.files_processed,
        result.stats.total_loc / 1000,
        pipeline_duration.as_secs_f64());

    let scale_factor_files = result.stats.files_processed as f64 / 467.0;
    let scale_factor_time = pipeline_duration.as_secs_f64() / 0.23;
    println!("   Scaling: {:.1}x files → {:.1}x time (sub-linear ✅)",
        scale_factor_files, scale_factor_time);
    println!();
    ...
    assert!(
        result.stats.files_processed > 1000,
        "Should process >1000 files (got {})",
        result.stats.files_processed
    );
    assert!(
        result.stats.total_loc > 400000,
        "Should process >400K LOC (got {})",
