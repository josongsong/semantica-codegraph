...
class LoRATrainer:
    ...
    def _initialize_lora_matrices(self) -> dict[str, any]:
        ...
        for module in self.config.target_modules:
            ...
            # A: (rank, input_dim)
            # B: (output_dim, rank)
            lora_weights[f"{module}_A"] = np.random.randn(self.config.rank, input_dim) * 0.01
    ...
    def _train_batch(
        ...
        for example in batch:
            ...
            query_emb = self._embed_with_lora(example.query, lora_weights, base_embedding_model)

            # Positive sample
            pos_emb = self._embed_with_lora(example.positive_chunk_id, lora_weights, base_embedding_model)

            # Negative samples
            neg_embs = [
                self._embed_with_lora(neg_id, lora_weights, base_embedding_model)
    ...
    def _embed_with_lora(self, text: str, lora_weights: dict, base_embedding_model) -> np.ndarray:
        """
    ...
    def _compute_contrastive_loss(
        ...
        # Positive similarity
        pos_sim = np.dot(query_emb, positive_emb) / (np.linalg.norm(query_emb) * np.linalg.norm(positive_emb))
