...
- Python 3.12+
- 4GB+ RAM
- (선택) Docker & Docker Compose - PostgreSQL 사용 시
...
┌────────────────▼────────────────────────┐
│   Infrastructure                         │
│  SQLite, Qdrant, Kùzu (All Embedded)    │
...
### 데이터베이스
- **SQLite** - 메타데이터 (기본, 설치 불필요)
- **PostgreSQL** - 메타데이터 (선택, 프로덕션)
- **Redis** - 캐시, 락 (선택)
- **Qdrant** - 벡터 DB (embedded 모드)
- **Kùzu** - 그래프 DB (embedded)
