...
from cwe.domain.ports import AnalysisResult, ConfusionMatrix, TaintAnalyzer  # noqa: E402

# Infrastructure imports (concrete implementations)
from cwe.infrastructure.schema_validator import YAMLSchemaValidator  # noqa: E402
...
class CWETestRunnerV2:
    ...
    def __init__(
        ...
        taint_service: TaintAnalyzer | None = None,
        ...
        self.cwe_root = cwe_root or settings.CWE_ROOT
        self.catalog_dir = self.cwe_root / "catalog"
        self.test_suite_dir = self.cwe_root / "test-suite"
        self.schema_dir = self.cwe_root / "schema"

        # Runtime Validation: Check for analyze(ir_doc) method
        # Note: We use analyze(ir_doc), not analyze_file(Path)
        if taint_service is not None and not hasattr(taint_service, "analyze"):
            raise TypeError(f"taint_service must have analyze() method. Got {type(taint_service).__name__}.")
    ...
    def create_with_defaults(cls, rules_base_path: Path | None = None) -> "CWETestRunnerV2":
        """
        Factory method: Create with production dependencies (SOTA Pattern).
        ...
        Design: Uses Builder pattern to reduce complexity
        """
        # Use Builder pattern for complex construction
        from cwe.infrastructure.taint_service_builder import TaintServiceBuilder

        builder = TaintServiceBuilder()
        taint_service = builder.build(rules_base_path)
    ...
    def _analyze_file(self, file_path: Path) -> AnalysisResult:
        """
        Analyze single file (sync wrapper) - tri-state result.
        ...
        - Never raises exceptions - returns ERROR on failure
        - Uses asyncio.run() to execute async code

        Note: If called from async context, use _analyze_file_async directly
        """
        try:
            import asyncio
            ...
            if "cannot be called from a running event loop" in str(e).lower():
                logger.error(f"Called from async context. Use _analyze_file_async() directly: {e}")
                return AnalysisResult.ERROR
            raise
