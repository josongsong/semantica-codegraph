...
from src.contexts.reasoning_engine.infrastructure.cache.rebuild_cache import CacheEntry, RebuildCache
...
class TestIncrementalBuilderWithCache:
    """IncrementalBuilder + Cache 통합"""

    def test_builder_with_cache(self, sample_graph):
        """Builder with cache"""
        cache = RebuildCache()
        builder = IncrementalBuilder(sample_graph, cache=cache)
    ...
    def test_cache_integration(self, sample_graph):
        ...
        # First build (miss)
        builder1 = IncrementalBuilder(sample_graph, cache=cache)
        ...
        # Second build (hit)
        builder2 = IncrementalBuilder(sample_graph, cache=cache)
