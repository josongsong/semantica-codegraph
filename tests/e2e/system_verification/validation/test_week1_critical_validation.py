...
async def test_integration_realistic():
    ...
    # 5. Context 검색 (Mock)
    adapter = ContextAdapter()
    relevant_code = await adapter.get_relevant_code("payment bug", "payment-service")
    assert "payment" in relevant_code.lower() or "Relevant Code" in relevant_code
    print(f"  ✅ Relevant code retrieved ({len(relevant_code)} chars)")

    # 6. 심볼 정의 조회
    symbol = await adapter.get_symbol_definition("process_payment", "payment-service")
    assert symbol["name"] == "process_payment"
    print(f"  ✅ Symbol definition: {symbol['name']} ({symbol['type']})")

    # 7. 영향 범위 파악
    impact = await adapter.get_impact_scope("src/payment/processor.py", "payment-service")
    assert isinstance(impact, list)
    print(f"  ✅ Impact scope: {len(impact)} files")
