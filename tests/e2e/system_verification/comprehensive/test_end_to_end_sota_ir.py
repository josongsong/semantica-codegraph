...
@pytest.mark.skip(reason="Requires LSP server setup - integration test")
async def test_sota_ir_full_pipeline(test_project: Path):
    ...
    """
    from codegraph_engine.code_foundation.infrastructure.ir.sota_ir_builder import SOTAIRBuilder
    ...
    # Note: repo_id/snapshot_id removed from build_full API
    result = await builder.build_full(files=python_files)
    ir_docs, global_ctx, retrieval_index = result[0], result[1], result[2]
    ...
    # Fuzzy search for "Calc" - use search_symbol with fuzzy=True
    fuzzy_results = retrieval_index.search_symbol("Calc", fuzzy=True, limit=5)
    assert len(fuzzy_results) > 0, "Fuzzy search returned no results"

    # Find Calculator in results (returns list of (Node, score) tuples)
    calc_results = [r for r in fuzzy_results if "Calculator" in (r[0].name or "")]
    assert len(calc_results) > 0, "Calculator not found in fuzzy search"

    # Get stats instead of important nodes (API doesn't have get_important_nodes)
    stats = retrieval_index.get_stats()
    assert stats["total_nodes"] > 0, "No nodes in stats"
    ...
    print(f"Unique FQNs: {stats['unique_fqns']}")
    print("=" * 60)
