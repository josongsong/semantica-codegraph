...
class TestRealWorldBenchmark:
    ...
    def test_real_incremental_rebuild(self):
        ...
        # Rebuild
        builder = ImpactAnalysisPlanner(graph)
    ...
    def test_real_cache_performance(self):
        ...
        builder1 = ImpactAnalysisPlanner(graph)
        start = time.time()
        ...
        # Run 2: Cache miss
        builder2 = ImpactAnalysisPlanner(graph, cache=cache)
        start = time.time()
        ...
        # Run 3: Cache hit
        builder3 = ImpactAnalysisPlanner(graph, cache=cache)
        start = time.time()
        ...
        for i in range(4, 11):
            builder = ImpactAnalysisPlanner(graph, cache=cache)
            builder.analyze_changes(changes)
            plan = builder.create_rebuild_plan()
