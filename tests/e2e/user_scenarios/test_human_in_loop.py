...
async def test_diff_approval_commit_flow():
    ...
    from apps.orchestrator.orchestrator.domain.approval_manager import ApprovalCriteria, ApprovalManager
...
"""
    ...
    # 1.3. 승인 (자동)
    print("\n1.3. 승인 (자동)...")
    approval_mgr = ApprovalManager(criteria=ApprovalCriteria(auto_approve_tests=False))  # 수동 승인
...
async def test_partial_approval():
    ...
    from apps.orchestrator.orchestrator.domain.approval_manager import ApprovalDecision, ApprovalSession
    from apps.orchestrator.orchestrator.domain.diff_manager import DiffManager
...
async def test_orchestrator_integration():
    ...
    from apps.orchestrator.orchestrator.adapters.llm.litellm_adapter import StubLLMProvider
    from apps.orchestrator.orchestrator.adapters.sandbox.stub_sandbox import LocalSandboxAdapter
    from apps.orchestrator.orchestrator.adapters.vcs.gitpython_adapter import StubVCSApplier
    from apps.orchestrator.orchestrator.adapters.workflow.langgraph_adapter import LangGraphWorkflowAdapter
    from apps.orchestrator.orchestrator.domain.approval_manager import ApprovalManager
    from apps.orchestrator.orchestrator.domain.diff_manager import DiffManager
    from apps.orchestrator.orchestrator.domain.incremental_workflow import IncrementalWorkflow
    from apps.orchestrator.orchestrator.domain.partial_committer import PartialCommitter
    from apps.orchestrator.orchestrator.orchestrator.fast_path_orchestrator import AgentOrchestrator
...
async def test_container_integration():
    ...
    print("\n4.1. container.py 파일 확인...")
    container_file = Path("packages/codegraph-shared/codegraph_shared/container.py")
