...
from codegraph_search.infrastructure.context_builder.models import (
    ContextChunk,
    ContextResult,
)
...
async def test_3_error_handling():
    """Test 3: 에러 핸들링 - 에러가 제대로 전파되는지 확인"""
    ...
    with pytest.raises(RuntimeError, match="Retrieval failed"):
        await adapter.get_relevant_code("query", "repo1")

    with pytest.raises(RuntimeError, match="Symbol search failed"):
        await adapter.get_symbol_definition("symbol", "repo1")

    print("  ✅ Errors properly propagated")
    print()
...
async def test_4_full_workflow_integration():
    ...
    # Workflow Step 3: 영향 범위 분석 (그래프 인덱스 없으면 빈 리스트 반환 가능)
    impact = await adapter.get_impact_scope(
        file_path=symbol["file_path"],
        repo_id="test-repo",
        snapshot_id="main",
    )
    # 그래프 인덱스 없을 때 빈 리스트 반환 - 정상 동작
    assert isinstance(impact, list)
    print(f"  ✅ Step 3: Impact scope ({len(impact)} files)")

    # Workflow Step 4: 관련 테스트 찾기 (그래프 인덱스 없으면 빈 리스트 반환 가능)
