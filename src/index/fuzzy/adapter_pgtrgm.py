...
class PostgresFuzzyIndex:
    ...
    def _extract_identifiers(self, doc: IndexDocument) -> list[tuple[str, str]]:
        ...
        if doc.metadata and "symbol_name" in doc.metadata:
            symbol_name = doc.metadata["symbol_name"]
            if symbol_name:
                identifiers.append((symbol_name, doc.node_type or "unknown"))

        # Extract from FQN parts
        if doc.fqn:
            parts = doc.fqn.split(".")
            for part in parts:
