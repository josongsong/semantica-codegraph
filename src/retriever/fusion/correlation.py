...
from .engine import FusedHit
from .weights import get_weights_for_intent
...
class CorrelationAwareFusion:
    ...
    def _apply_correlation_adjustments(
        self, base_score: float, sources: dict[str, float]
    ) -> float:
        """
        ...
        if vector_score >= 0.85:
            ...
            if other_scores_low:
                ...
                logger.debug(
                    f"Semantic drift penalty: vector={vector_score:.2f}, "
                    f"others<0.2 (*{self.SEMANTIC_DRIFT_PENALTY})"
                )
