...
class InterleavingWeightsV2:
    ...
    def linear_combination(
        profiles: list[tuple["InterleavingWeightsV2", float]]
    ...
    def for_code_search() -> "InterleavingWeightsV2":
        ...
        return InterleavingWeightsV2(
            vector_weight=0.5, lexical_weight=0.3, symbol_weight=0.1, graph_weight=0.1
        )
    ...
    def for_symbol_navigation() -> "InterleavingWeightsV2":
        ...
        return InterleavingWeightsV2(
            vector_weight=0.2, lexical_weight=0.2, symbol_weight=0.5, graph_weight=0.1
        )
    ...
    def for_flow_trace() -> "InterleavingWeightsV2":
        ...
        return InterleavingWeightsV2(
            vector_weight=0.2, lexical_weight=0.1, symbol_weight=0.2, graph_weight=0.5
        )
    ...
    def for_concept_search() -> "InterleavingWeightsV2":
        ...
        return InterleavingWeightsV2(
            vector_weight=0.7, lexical_weight=0.2, symbol_weight=0.05, graph_weight=0.05
        )
    ...
    def balanced() -> "InterleavingWeightsV2":
        ...
        return InterleavingWeightsV2(
            vector_weight=0.4, lexical_weight=0.3, symbol_weight=0.2, graph_weight=0.1
        )
...
class IntentScore:
    ...
    def to_weights(self) -> InterleavingWeightsV2:
        ...
        if self.symbol_like > 0:
            profiles.append(
                (InterleavingWeightsV2.for_symbol_navigation(), self.symbol_like)
            )
        if self.flow_like > 0:
            profiles.append((InterleavingWeightsV2.for_flow_trace(), self.flow_like))
        if self.concept_like > 0:
            profiles.append(
                (InterleavingWeightsV2.for_concept_search(), self.concept_like)
            )
        if self.code_search_like > 0:
            profiles.append(
                (InterleavingWeightsV2.for_code_search(), self.code_search_like)
            )
...
class SmartInterleaverV2:
    ...
    def _build_chunk_index(
        ...
        chunk_index = defaultdict(
            lambda: {"appearances": [], "chunk": None, "original_scores": []}
        )
        ...
        for result in strategy_results:
            for rank, chunk in enumerate(result.chunks):
                ...
                chunk_index[chunk_id]["appearances"].append(
                    (result.strategy, original_score, rank)
                )
    ...
    def _compute_rrf_scores(
        ...
        for chunk_id, info in chunk_index.items():
            ...
            for strategy, original_score, rank in info["appearances"]:
                ...
                max_component_score = max(max_component_score, rrf_component)

            # Quality-aware consensus boost
            num_strategies = len(info["appearances"])
            consensus_factor = self._compute_consensus_factor(
                num_strategies, max_component_score
            )
    ...
    def _compute_consensus_factor(
        self, num_strategies: int, max_component_score: float
    ) -> float:
        """
        ...
        base_factor = 1.0 + self.consensus_boost_base * math.sqrt(
    ...
    def set_weights_for_intent(self, intent: str) -> None:
        ...
        if "symbol" in intent_lower or "definition" in intent_lower:
            ...
            "flow" in intent_lower or "trace" in intent_lower or "call" in intent_lower
        ):
...
def example_usage():
    ...
    interleaver2 = InterleaverFactoryV2.create(
        method="weighted_rrf", intent_scores=intent_scores
    )

    interleaved2 = interleaver2.interleave(
        [vector_results, lexical_results, symbol_results], top_k=10
    )
