...
class ScoreNormalizer:
    ...
    - Lexical (Zoekt): arbitrary positive scores
    - Vector (Qdrant): cosine similarity (0-1) or distance metrics
    - Symbol (Kuzu): binary or count-based scores
    ...
    def normalize_hits(self, hits: list[SearchHit], source: str) -> list[SearchHit]:
        ...
        if source == "lexical":
            ...
            logger.warning(f"Unknown source '{source}', using identity normalization")
            return hits
    ...
    def _normalize_lexical(self, hits: list[SearchHit]) -> list[SearchHit]:
        ...
        Zoekt returns arbitrary positive scores based on TF-IDF.
        """
        if not hits:
            return []
        ...
        for hit in hits:
            ...
            normalized_hit = hit.model_copy()
            normalized_hit.score = normalized_score
