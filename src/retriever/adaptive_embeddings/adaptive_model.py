...
from .models import RepoAdaptation
...
class AdaptiveEmbeddingModel:
    ...
    def embed(
        self, text: str, repo_id: str | None = None
    ) -> np.ndarray:
        """
    ...
    def embed_batch(
        self, texts: list[str], repo_id: str | None = None
    ) -> list[np.ndarray]:
        """
    ...
    def _apply_lora(
        self, base_emb: np.ndarray, lora_weights: dict[str, any]
    ) -> np.ndarray:
        """
...
class AdaptiveSearchWrapper:
    ...
    async def search(
        ...
        for result in results:
            result["used_adaptation"] = (
                use_adaptation and self.adaptive_model.is_adapted(repo_id)
            )
