...
class OptimizedRetrieverService:
    ...
    def __init__(
        ...
        llm_reranker_cache: Any | None = None,
        dependency_orderer: Any | None = None,
        query_expander: Any | None = None,
        # P1 components
        learned_reranker: Any | None = None,
        smart_interleaver: Any | None = None,
    ...
    async def retrieve(
        ...
        pipeline_stages["query_analysis"] = (time.time() - stage_start) * 1000
        ...
        pipeline_stages["query_expansion"] = (time.time() - stage_start) * 1000
        ...
        if self.config.enable_parallel_strategies:
            # Parallel retrieval
            strategy_results = await self._retrieve_parallel(
                expanded_query, adaptive_k
            )
        else:
            # Sequential retrieval
            strategy_results = await self._retrieve_sequential(
                expanded_query, adaptive_k
            )

        pipeline_stages["multi_strategy_retrieval"] = (
        ...
        if self.config.use_smart_interleaving and self.smart_interleaver:
            ...
            interleaved_chunks = self.smart_interleaver.interleave(
                formatted_results, top_k=adaptive_k
            )
        else:
        ...
        pipeline_stages["smart_interleaving"] = (time.time() - stage_start) * 1000
        ...
        pipeline_stages["learned_reranker"] = (time.time() - stage_start) * 1000

        # Stage 6: Dependency Ordering
        stage_start = time.time()
        if self.config.use_dependency_ordering and self.dependency_orderer:
            ordered_chunks = self.dependency_orderer.order_by_dependencies(
                reranked_chunks[:30]  # Top 30 for dependency analysis
            )
        else:
            ordered_chunks = reranked_chunks

        pipeline_stages["dependency_ordering"] = (time.time() - stage_start) * 1000

        # Stage 7: Cross-encoder (final top-k)
        stage_start = time.time()
        if self.config.use_cross_encoder and self.cross_encoder:
            final_k = self.config.cross_encoder_final_k
            final_chunks = await self.cross_encoder.rerank(
                query, ordered_chunks[:20], top_k=final_k
            )
        else:
            final_chunks = ordered_chunks[: (top_k or adaptive_k)]

        pipeline_stages["cross_encoder"] = (time.time() - stage_start) * 1000
        ...
        logger.info(
            f"Retrieval complete: {len(final_chunks)} chunks, {total_latency:.0f}ms"
        )
    ...
    def create_optimized(
        ...
        **kwargs: Any,
        ...
        if config.use_embedding_cache:
            from src.retriever.hybrid.late_interaction_optimized import (
                EmbeddingCache,
            )
        ...
        if config.use_dependency_ordering:
            from src.retriever.context_builder.dependency_ordering import (
        ...
        if config.use_smart_interleaving:
            if config.fusion_version == "v2":
                ...
                from src.retriever.fusion.smart_interleaving import SmartInterleaver
        ...
        if config.use_adaptive_topk:
            from src.retriever.adaptive.topk_selector import (
        ...
        if config.use_cross_encoder:
            from src.retriever.hybrid.cross_encoder_reranker import (
