...
class DependencyGraphExtractor:
    ...
    def extract_dependencies(
        ...
        for chunk in chunks:
            ...
            dependencies[chunk_id] = DependencyInfo(
                chunk_id=chunk_id, depends_on=depends_on, depended_by=[]
            )
    ...
    def _build_chunk_symbol_mapping(
        self, chunks: list[dict[str, Any]]
    ) -> dict[str, list[str]]:
        """
    ...
    def _calculate_dependency_levels(
        ...
        to_process = deque(
            [
                chunk_id
                for chunk_id, info in dependencies.items()
        ...
        while to_process:
            ...
            current_level = dependencies[chunk_id].level
            ...
            for dependent_id in dependencies[chunk_id].depended_by:
                ...
                all_deps_processed = all(
                    dep_id in processed for dep_id in dep_info.depends_on
                )
...
class DependencyAwareOrdering:
    ...
    def order_chunks(self, chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for scc in scc_order:
            ...
            for chunk_id in scc:
                if chunk_id in chunk_map:
                    scc_chunks.append(
                        (dependencies[chunk_id].level, chunk_id, chunk_map[chunk_id])
                    )
        ...
        logger.info(
            f"Dependency ordering: {len(chunks)} chunks → "
            f"{len(ordered_chunks)} ordered ({len(sccs)} dependency groups)"
        )
    ...
    def _topological_sort_sccs(
        self, sccs: list[list[str]], dependencies: dict[str, DependencyInfo]
    ) -> list[list[str]]:
        """
    ...
    def get_ordering_stats(
        self, original_chunks: list[dict], ordered_chunks: list[dict]
    ) -> dict[str, Any]:
        """
        ...
        chunks_with_deps = sum(
            1 for info in dependencies.values() if info.depends_on
        )
        ...
        reordering_distance = sum(
            1 for i, chunk_id in enumerate(ordered_ids) if original_ids[i] != chunk_id
        )
        ...
        return {
            ...
            "avg_dependencies_per_chunk": (
                total_deps / len(ordered_chunks) if ordered_chunks else 0
            ),
            "reordering_distance": reordering_distance,
            "reordering_percentage": (
                reordering_distance / len(ordered_chunks) * 100
                if ordered_chunks
                else 0
            ),
        }
