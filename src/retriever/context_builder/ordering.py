...
from src.retriever.intent import IntentKind

if TYPE_CHECKING:
    from src.ports import GraphPort
...
class ChunkOrderingStrategy:
    ...
    def _order_by_flow(self, chunks: list[dict[str, Any]]) -> list[OrderedChunk]:
        ...
        remaining_ids = set(c["chunk_id"] for c in chunks) - set(
            o.chunk_id for o in ordered
        )
        for chunk in chunks:
        ...
        logger.info(
            f"Flow ordering: {len(ordered)} chunks, "
            f"{len(ordered) - len(remaining_ids)} in flow"
        )
    ...
    def _order_by_structure(self, chunks: list[dict[str, Any]]) -> list[OrderedChunk]:
        ...
        logger.info(
            f"Structural ordering: {len(definitions)} definitions, "
            f"{len(usages)} usages, {len(others)} others"
        )

        return ordered

    def _order_by_semantic_relevance(
    ...
    def _order_by_score(
        self, chunks: list[dict[str, Any]], reason: str = "score"
    ) -> list[OrderedChunk]:
        """
    ...
    def _is_definition_chunk(self, chunk: dict[str, Any]) -> bool:
        ...
        chunk_type = chunk.get("chunk_type", "")
        return any(
            kw in chunk_type.lower()
            for kw in ["definition", "class", "function", "method"]
        )
