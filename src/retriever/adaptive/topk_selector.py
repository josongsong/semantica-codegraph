...
class QueryAnalyzer:
    ...
    def __init__(self):
        ...
        self.code_pattern = re.compile(
            r"[A-Z][a-z]+(?:[A-Z][a-z]+)*|[a-z_][a-z0-9_]*"
        )
    ...
    def analyze(self, query: str) -> QueryComplexity:
        ...
        has_natural_language = any(
            len(token) > 4 and token.isalpha() for token in tokens
        )
        ...
        if has_code_identifiers:
            ...
            camel_case_count = sum(
                1 for m in code_matches if m[0].isupper() and len(m) > 1
            )
        ...
        # Average specificity
        specificity_score = (
            np.mean(specificity_factors) if specificity_factors else 0.5
        )
...
class AdaptiveTopKSelector:
    ...
    def refine_k_from_scores(
        self, scores: list[float], initial_k: int
    ) -> int:
        """
        ...
        high_quality_count = np.sum(
            scores_arr >= self.config.quality_score_threshold
        )
        if high_quality_count > initial_k * 0.8:
            # Most results are high quality, consider expanding
            cutoff_k = min(
                int(initial_k * 1.5), self.config.max_k, len(scores)
            )
            logger.info(
                f"High quality results ({high_quality_count}), expanding k to {cutoff_k}"
            )
...
class TwoStageRetrieval:
    ...
    async def retrieve(
        ...
        adaptive_k = self.selector.select_adaptive_k(
            query, intent, available_scores=scores
        )

        logger.info(
            f"Stage 2: Selected adaptive k={adaptive_k} from {len(stage1_results)} candidates"
        )
...
class BudgetAwareSelector:
    ...
    def select_with_budget(
        ...
        latency_limit_k = int(
            self.max_latency_ms / self.latency_per_candidate_ms
        )
