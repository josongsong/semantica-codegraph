...
if TYPE_CHECKING:
    ...
    from src.retriever.multi_index import MultiIndexOrchestrator
...
class ReasoningRetriever:
    ...
    async def retrieve_with_reasoning(
        ...
        logger.info(
            f"Strategy planned: {len(strategy.steps)} steps "
            f"(difficulty={strategy.estimated_difficulty})"
        )
        ...
        for step in strategy.steps[: self.max_reasoning_steps]:
            ...
            result = await self._execute_search_step(
                repo_id, snapshot_id, step
            )
    ...
    async def _execute_search_step(
        self, repo_id: str, snapshot_id: str, step: ReasoningStep
    ) -> dict:
        """Execute a single search step."""
        # Map tool to index
        llm_requested_indices = [step.tool.value]
    ...
    def _build_trace(self, strategy: SearchStrategy, step_results: list[dict]) -> str:
        ...
        for step, result in zip(strategy.steps, step_results):
