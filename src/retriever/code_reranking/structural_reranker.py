...
from .models import ASTSimilarity, CodeRerankedChunk, StructuralFeature
...
class StructuralReranker:
    ...
    def rerank(
        ...
        for candidate in candidates:
            ...
            if reference_ast and code_content:
                ast_similarity = self._compute_similarity(
                    reference_ast, code_content
                )
                structural_score = ast_similarity.overall_score
            else:
                # No reference code, use pattern matching on query
                ast_similarity = self._match_query_patterns(
                    code_content, query_context or ""
                )
                structural_score = ast_similarity.overall_score
    ...
    def _compute_similarity(
        self, reference_ast: ast.AST, candidate_code: str
    ) -> ASTSimilarity:
    ...
    def _extract_features(self, tree: ast.AST) -> dict[StructuralFeature, set]:
        ...
        for node in ast.walk(tree):
            ...
            # Class hierarchy
                ...
                features[StructuralFeature.CLASS_HIERARCHY].add(
                    f"{node.name}:{','.join(bases)}"
                )

            # Control flow
            elif isinstance(node, (ast.If, ast.For, ast.While, ast.Try)):
                features[StructuralFeature.CONTROL_FLOW].add(
    ...
    def _match_query_patterns(
        self, code_content: str, query: str
    ) -> ASTSimilarity:
        """
        ...
        if any(
            kw in query_lower
            for kw in ["function", "def", "method", "call", "invoke"]
        ):
        ...
        explanation = f"Matched patterns: {', '.join(matched_patterns)}" if matched_patterns else "No strong pattern match"
    ...
    def _generate_explanation(
        self, feature_scores: dict, matched_patterns: list[str]
    ) -> str:
