...
from .models import CallGraphProximity, CodeRerankedChunk
    ...
    def _calculate_proximity(
        ...
        for ref_func in reference_functions:
            for cand_func in candidate_functions:
                ...
                proximity = self._check_direct_relationship(
                    ref_func, cand_func, call_graph_adapter
                )
...
class MockCallGraphAdapter:
    """
    Mock adapter for testing (when real graph not available).

    Replace with actual adapter that queries Kuzu graph.
    """

    def __init__(self):
        """Initialize mock adapter."""
        # Example call graph: func_a -> func_b -> func_c
        self.edges = {
            "func_a": ["func_b"],
