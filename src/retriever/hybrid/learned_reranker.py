...
class FeatureExtractor:
    ...
    def extract(
        self, query: str, chunk: dict[str, Any]
    ) -> RerankerFeatures:
        """
        ...
        query_has_natural_language = any(
            len(token) > 4 and token.isalpha() for token in query_tokens
        )
        ...
        keyword_overlap = (
            exact_token_matches / len(query_tokens_set)
            if query_tokens_set
            else 0.0
        )
        ...
        code_identifier_overlap = (
            len(query_identifiers & chunk_identifiers) / len(query_identifiers)
        ...
        is_config_file = any(
            ...
            for cfg in ["config", "settings", ".json", ".yaml"]
        )
...
class LearnedReranker:
    ...
    def train(
        ...
        try:
            ...
            for idx in top_features:
                logger.info(f"  Feature {idx}: {importances[idx]:.3f}")
            ...
            logger.warning(
                "scikit-learn not available, using simple heuristic model"
            )
    ...
    def rerank(
        self, query: str, candidates: list[dict[str, Any]], top_k: int = 50
    ) -> list[dict[str, Any]]:
        """
    ...
    def collect_training_example(
        self, query: str, chunk: dict[str, Any], llm_score: float
    ) -> None:
        """
        ...
        self.training_data.append(
            {"query": query, "chunk": chunk, "llm_score": llm_score}
        )
    ...
    def save_training_data(self, path: str) -> None:
        ...
        logger.info(
            f"Saved {len(self.training_data)} training examples to {path}"
        )
    ...
    def load_training_data(self, path: str) -> None:
        ...
        logger.info(
            f"Loaded {len(self.training_data)} training examples from {path}"
        )
...
class HybridReranker:
    ...
    async def rerank(
        ...
        use_llm = (
            ...
            or random.random() < self.llm_fallback_rate
        )

        if use_llm and self.llm_reranker:
            # LLM fallback
            logger.info(
                f"Using LLM fallback (confidence: {max_score:.2f}, "
            ...
            for candidate in llm_results[:top_k]:
                ...
                self.learned_reranker.collect_training_example(
                    query, candidate, score_value
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "learned_rate": (
                self.learned_used / self.total_queries
                if self.total_queries > 0
                else 0.0
            ),
            "cost_savings": (
                (self.learned_used / self.total_queries * 0.95)
                if self.total_queries > 0
                else 0.0
            ),
        }
