...
from src.retriever.hybrid.llm_reranker import LLMReranker, LLMScore
...
class LLMScoreCachePort(Protocol):
    ...
    def set(
        self, cache_key: str, score: LLMScore, ttl_seconds: int | None = None
    ) -> None:
...
class FileBasedLLMScoreCache:
    ...
    def get(self, cache_key: str) -> CachedLLMScore | None:
        ...
        try:
            with open(cache_file, "rb") as f:
                cached: CachedLLMScore = pickle.load(f)
    ...
    def set(
        ...
        cached = CachedLLMScore(
            score=score, cached_at=time.time(), cache_version="v1"
        )
...
class CachedLLMReranker(LLMReranker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cache = (
            cache if cache is not None else InMemoryLLMScoreCache(maxsize=10000)
        )
        ...
        logger.info(
            f"CachedLLMReranker initialized (cache_ttl={cache_ttl}s, "
            f"cache_type={type(self.cache).__name__})"
        )
