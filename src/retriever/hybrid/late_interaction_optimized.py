...
class OptimizedLateInteractionSearch:
    ...
    async def search(
        ...
        for i, (chunk_id, _) in enumerate(doc_embeddings_list):
            ...
            candidate["score"] = (
                candidate.get("score", 0.0) * 0.5 + scores[i] * 0.5
            )  # Blend with original
    ...
    def _maxsim_cpu_batch(
        self, query_embs: np.ndarray, doc_embs_list: list[tuple[str, np.ndarray]]
    ) -> list[float]:
    ...
    def _maxsim_gpu_batch(
        ...
        for i in range(0, len(doc_embs_list), batch_size):
            ...
            for chunk_id, doc_embs in batch:
