...
class QueryRewriter:
    ...
    def rewrite(
        self, query: str, intent: IntentKind, language_hint: str | None = None
    ) -> RewrittenQuery:
        """
    ...
    def _extract_code_identifiers(self, tokens: list[str]) -> list[str]:
        ...
        for token in tokens:
            ...
            elif "." in token and all(
                part.replace("_", "").isalnum() for part in token.split(".")
            ):
                identifiers.append(token)
    ...
    def explain(self, rewritten: RewrittenQuery) -> str:
        ...
        if rewritten.removed_stopwords:
            parts.append(
                f"Removed stopwords: {', '.join(rewritten.removed_stopwords[:5])}"
            )
