...
class CodebaseVocabulary:
    ...
    def _extract_terms(self, content: str) -> list[tuple[str, str]]:
        ...
        ts_classes = re.findall(
            r"(?:class|interface)\s+([A-Z][a-zA-Z0-9_]*)", content
        )
        terms.extend((name, "class") for name in ts_classes)

        # Variables (less reliable, limit to assignments)
        # Python: variable_name =
        var_assigns = re.findall(
            r"^\s*([a-z_][a-z0-9_]*)\s*=\s*", content, re.MULTILINE
        )
    ...
    def find_similar_terms(
        ...
        term_embs_norm = self.term_embeddings / (
            np.linalg.norm(self.term_embeddings, axis=1, keepdims=True) + 1e-8
        )
    ...
    def get_cooccurring_terms(
        self, term: str, top_k: int = 5
    ) -> list[tuple[str, int]]:
        """
...
class ContextualQueryExpander:
    ...
    def expand(
        ...
        similar_terms = [
            ...
            if self.vocabulary.terms[term].frequency >= frequency_min
        ]
    ...
    def explain(self, expansion_result: dict[str, Any]) -> str:
        ...
        for term, score in expansion_result["expansion_terms"][:5]:
            ...
            lines.append(
                f"  - {term} (score: {score:.3f}, freq: {term_info.frequency}, "
                f"type: {term_info.term_type})"
            )
