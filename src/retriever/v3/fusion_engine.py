...
from .config import RetrieverV3Config, WeightProfile
from .consensus_engine import ConsensusEngine
from .models import ConsensusStats, FeatureVector, FusedResultV3, IntentProbability, RankedHit
from .rrf_normalizer import RRFNormalizer
...
class FusionEngineV3:
    ...
    def fuse(
        ...
        logger.debug(f"Intent weights: {weights.to_dict()}")

        # Step 2: RRF normalization and weighting
        base_scores, rrf_scores = self.rrf_normalizer.normalize_and_weight(
            hits_by_strategy, weights
        )
        logger.debug(f"Base scores calculated for {len(base_scores)} chunks")
        ...
        if query_expansions:
            ...
            logger.debug(f"Query expansion boost applied for {len(query_expansions)} terms")

        # Step 3: Consensus boosting
        final_scores, consensus_stats = self.consensus_engine.apply_consensus_boost(
            base_scores, hits_by_strategy
        )
        logger.debug(f"Consensus boost applied to {len(final_scores)} chunks")
        ...
        logger.info(
            ...
            f"dominant_intent={intent_prob.dominant_intent()}"
        )
    ...
    def _calculate_intent_weights(self, intent_prob: IntentProbability) -> WeightProfile:
        ...
        if dominant == "flow" and intent_prob.flow > 0.2:
            ...
            logger.debug(f"Flow intent boost applied: graph weight *= {boost_factor}")

        elif dominant == "symbol" and intent_prob.symbol > 0.3:
            # Boost symbol weight for symbol queries
            boost_factor = 1.2
            combined["sym"] *= boost_factor
            logger.debug(f"Symbol intent boost applied: symbol weight *= {boost_factor}")
    ...
    def _apply_expansion_boost(
        ...
        if boosted_chunks > 0:
            logger.debug(f"Expansion boost applied to {boosted_chunks} chunks")
        ...
        for result in results:
            ...
            for strategy in ["vector", "lexical", "symbol", "graph"]:
                ...
                if rrf_score > 0:
                    ...
                    strategy_contribs.append(
                        f"{strategy}={contrib:.4f} (rrf={rrf_score:.4f}, w={weight:.2f})"
                    )
    ...
    def apply_cutoff(
        self, results: list[FusedResultV3], intent_prob: IntentProbability
    ) -> list[FusedResultV3]:
        """
        ...
        logger.debug(f"Applying cutoff k={k} for intent={dominant}")
