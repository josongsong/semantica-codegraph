...
from .models import ConsensusStats, RankedHit
...
class ConsensusEngine:
    ...
    def calculate_consensus_stats(
        self, chunk_id: str, hits_by_strategy: dict[str, list[RankedHit]]
    ) -> ConsensusStats:
        """
    ...
    def get_chunks_by_consensus(
        self, consensus_stats: dict[str, ConsensusStats], min_strategies: int = 2
    ) -> list[str]:
        """
        ...
        filtered = [
            (chunk_id, stats)
            for chunk_id, stats in consensus_stats.items()
            if stats.num_strategies >= min_strategies
        ]
