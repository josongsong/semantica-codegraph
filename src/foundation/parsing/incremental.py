...
class EditCalculator:
    ...
    def calculate_edits(
        self, old_content: str, diff_hunks: list[DiffHunk]
    ) -> list[Edit]:
        """
        ...
        for hunk in diff_hunks:
            ...
            # For this we need to apply the diff to know the new content
            new_end_line = hunk.new_start - 1 + hunk.new_count
            # Approximate new_end_col (assume same as old for simplicity)
            new_end_col = old_end_col
            new_end_byte = start_byte + (old_end_byte - start_byte) + (
