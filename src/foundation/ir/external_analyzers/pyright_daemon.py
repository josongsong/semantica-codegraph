...
from .snapshot import PyrightSemanticSnapshot, Span
...
class PyrightSemanticDaemon:
    ...
    def export_semantic_incremental(
        ...
        keys_to_remove = [
            key for key in new_typing_info.keys()
            if key[0] in changed_file_strs
        ]
        ...
        if deleted_files:
            deleted_file_strs_set = set(str(f) for f in deleted_files)
    ...
    async def export_semantic_for_files_async(
        ...
        async def _hover_single(
            file_path: Path, line: int, col: int
        ) -> tuple[str, Span, str | None]:
            """Execute single hover with concurrency limit."""
            async with semaphore:
                try:
                    result = await asyncio.to_thread(
