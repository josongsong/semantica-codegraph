...
from src.contexts.multi_index.infrastructure.vector.priority import get_chunk_priority
...
class EmbeddingQueue:
    ...
    def __init__(
        ...
        vector_index,
        chunk_store=None,
        worker_pool=None,  # EmbeddingWorkerPool (선택)
    ):
        """
    ...
    async def get_stats(self, repo_id: str | None = None, snapshot_id: str | None = None) -> dict:
        ...
        if repo_id:
            where_clause += " AND repo_id = $1"
            params.append(repo_id)
            if snapshot_id:
                where_clause += " AND snapshot_id = $2"
