...
class ImplementationMode(BaseModeHandler):
    ...
    async def _generate_code(self, task: Task, related_code: str, context: ModeContext) -> str:
        ...
        if not self.llm:
            ...
            self.logger.warning("No LLM client provided, using fallback generation")
            from src.contexts.agent_automation.infrastructure.fallback import SimpleLLMFallback

            file_path = context.current_files[0] if context.current_files else "generated.py"
            return SimpleLLMFallback.generate_implementation(task.query, file_path)
