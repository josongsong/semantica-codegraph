...
@mode_registry.register(AgentMode.CONTEXT_NAV)
class ContextNavigationMode(BaseModeHandler):
    ...
    async def execute(self, task: Task, context: ModeContext) -> Result:
        ...
        return self._create_result(
            data={
                ...
                "files": list({r.get("file_path") for r in results if "file_path" in r}),
            },
            trigger=trigger,
    ...
    async def exit(self, context: ModeContext) -> None:
        ...
        self.logger.info(
            f"Exiting navigation - found {len(context.current_files)} files, {len(context.current_symbols)} symbols"
        )
        await super().exit(context)


@mode_registry.register(AgentMode.CONTEXT_NAV, simple=True)
