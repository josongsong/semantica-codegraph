...
class SmartInterleaver:
    ...
    def interleave(
        ...
        for _chunk_id, info in chunks_with_consensus.items():
            weighted_score = 0.0
            ...
            if len(info["appearances"]) > 1:
                consensus_factor = 1.0 + self.consensus_boost * (len(info["appearances"]) - 1)
    ...
    def _normalize_scores(self, strategy_results: list[StrategyResult]) -> list[StrategyResult]:
    ...
    def _build_chunk_index(self, strategy_results: list[StrategyResult]) -> dict[str, dict[str, Any]]:
        ...
        for result in strategy_results:
            for rank, chunk in enumerate(result.chunks):
                ...
                chunk_index[chunk_id]["appearances"].append((result.strategy, score, rank))
    ...
    def _compute_consensus_scores(self, chunk_index: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
...
class RoundRobinInterleaver:
    ...
    def interleave(
        ...
        logger.info(f"Round-robin interleaved {len(strategy_results)} strategies → {len(interleaved)} chunks")
