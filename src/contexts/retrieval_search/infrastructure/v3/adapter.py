...
from src.contexts.retrieval_search.infrastructure.context_builder import ContextBuilder, ContextChunk, ContextResult
from src.contexts.retrieval_search.infrastructure.multi_index import MultiIndexResult
from src.contexts.retrieval_search.infrastructure.v3.config import RetrieverV3Config
from src.contexts.retrieval_search.infrastructure.v3.models import FusedResultV3, IntentProbability
...
class V3RetrieverAdapter:
    ...
    def fuse_multi_index_result(
        ...
        logger.info(
            ...
            strategies_used=len(hits_by_strategy),
        )
        record_histogram("retriever_adapter_fusion_results", len(fused_results))
    ...
    def build_context(
        ...
        if not self.context_builder:
            logger.warning("context_builder_not_available", message="Context builder instance is None")
            return None
        ...
        logger.info("building_context", results_count=len(fused_results), token_budget=token_budget)
        ...
        logger.info(
            ...
            token_budget=token_budget,
            token_utilization=context.total_tokens / token_budget if token_budget > 0 else 0.0,
        )
        record_histogram("retriever_adapter_context_chunks", context.chunk_count)
        record_histogram("retriever_adapter_context_tokens", context.total_tokens)
