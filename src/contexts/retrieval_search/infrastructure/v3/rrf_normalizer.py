...
from src.contexts.retrieval_search.infrastructure.v3.config import RRFConfig, WeightProfile
from src.contexts.retrieval_search.infrastructure.v3.models import RankedHit
...
class RRFNormalizer:
    ...
    def calculate_rrf_scores(self, hits_by_strategy: dict[str, list[RankedHit]]) -> dict[str, dict[str, float]]:
        """
        ...
        for strategy, hits in hits_by_strategy.items():
            ...
            for hit in hits:
                ...
                # Direct calculation is faster than lookup tables on modern CPUs
                # (Benchmarked: 0.0127ms vs 0.0178ms per query with 200 calculations)
    ...
    def get_strategy_rrf_score(self, chunk_id: str, strategy: str, rrf_scores: dict[str, dict[str, float]]) -> float:
        """
