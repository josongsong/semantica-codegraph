...
from src.contexts.retrieval_search.infrastructure.observability.models import Explanation, SourceBreakdown
...
class RetrievalExplainer:
    ...
    def explain_result(
        ...
        # Generate reasoning
        reasoning = self._generate_reasoning(chunk_id, final_score, breakdown, matched_terms, metadata)
    ...
    def _generate_reasoning(
        ...
        if breakdown:
            ...
            source_names = ", ".join(f"{s.source} ({s.contribution:.3f})" for s in top_sources)
    ...
    def explain_ranking(self, results: list[dict], top_k: int = 10) -> list[Explanation]:
        """
    ...
    def compare_results(self, result_a: Explanation, result_b: Explanation) -> dict[str, any]:
        """
        ...
        sorted_diffs = sorted(source_diffs.items(), key=lambda x: abs(x[1]), reverse=True)

        # Generate comparison reasoning
        reasons = []
        for source, diff in sorted_diffs[:3]:
            if diff > 0:
                reasons.append(f"{source} contributed {diff:.3f} more to result A")
            else:
                reasons.append(f"{source} contributed {abs(diff):.3f} more to result B")
