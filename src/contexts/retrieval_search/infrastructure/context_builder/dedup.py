...
class Deduplicator:
    ...
    def deduplicate(self, fused_hits: "list[FusedHit]") -> "list[FusedHit]":
        ...
        for hit in fused_hits:
            ...
            if file_path in selected_ranges:
                overlap_ratio = self._calculate_max_overlap(start_line, end_line, selected_ranges[file_path])

                if overlap_ratio >= 1.0 and self.drop_on_full_overlap:
                    # Fully contained in another chunk → drop
                    logger.debug(
                        f"Dropping fully overlapping chunk: {hit.chunk_id} ({file_path}:{start_line}-{end_line})"
                    )
                    dropped_count += 1
                    continue
                ...
                if overlap_ratio >= self.overlap_threshold:
                    ...
                    penalized_count += 1
                    logger.debug(f"Penalizing overlapping chunk: {hit.chunk_id} (overlap={overlap_ratio:.2f})")
    ...
    def _calculate_max_overlap(self, start: int, end: int, existing_ranges: list[tuple[int, int, str]]) -> float:
        """
        ...
        if chunk_size <= 0:
            logger.warning(f"Invalid chunk range: start={start}, end={end}, size={chunk_size}")
            return 0.0
