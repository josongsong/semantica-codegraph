...
from src.contexts.retrieval_search.infrastructure.query.models import DecomposedQuery, QueryStep, QueryType
...
class QueryDecomposer:
    ...
    async def decompose(self, query: str) -> DecomposedQuery:
        ...
        try:
            # Call LLM
            response_text = await self.llm_client.generate(prompt, max_tokens=800, temperature=0.3)
            ...
            logger.info(f"Decomposed into {len(decomposed.steps)} steps (type={decomposed.query_type.value})")
