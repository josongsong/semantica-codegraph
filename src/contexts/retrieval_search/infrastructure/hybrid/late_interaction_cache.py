...
class InMemoryEmbeddingCache:
    ...
    def set(self, chunk_id: str, embeddings: np.ndarray, metadata: dict | None = None) -> None:
        ...
        self._cache[chunk_id] = CachedEmbedding(chunk_id=chunk_id, embeddings=embeddings, metadata=metadata)
...
class OptimizedLateInteraction:
    ...
    def search(
        ...
        logger.info(f"Late interaction: {len(candidates)} → {top_k} chunks (cache hit rate: {hit_rate:.1f}%)")
    ...
    def precompute_embeddings(
        ...
        logger.info(f"Pre-computation complete: {len(self.cache)} embeddings cached")
