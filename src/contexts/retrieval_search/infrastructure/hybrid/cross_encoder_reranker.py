...
class CrossEncoderReranker:
    ...
    async def rerank(
        ...
        logger.info(f"Cross-encoder reranking {len(candidates_to_rerank)} candidates")
        ...
        for i, candidate in enumerate(candidates_to_rerank):
            ...
            candidate["final_score"] = 0.8 * cross_score_normalized + 0.2 * original_score
    ...
    def predict_batch(self, query_document_pairs: list[tuple[str, str]]) -> list[float]:
        """
...
class CachedCrossEncoderReranker:
    ...
    async def rerank(
        ...
        if uncached_candidates:
            scored_uncached = await self.reranker.rerank(query, uncached_candidates, top_k=len(uncached_candidates))
        ...
        # Sort by final score
        reranked = sorted(cached_scores, key=lambda c: c["final_score"], reverse=True)

        cache_hit_rate = self.hits / (self.hits + self.misses) if (self.hits + self.misses) > 0 else 0.0
        logger.info(f"Cross-encoder cache: {self.hits} hits, {self.misses} misses (hit rate: {cache_hit_rate:.1%})")
...
class HybridFinalReranker:
    ...
    async def rerank(self, query: str, candidates: list[dict[str, Any]], top_k: int = 10) -> list[dict[str, Any]]:
        ...
        if self.learned_reranker and len(current_candidates) > 20:
            ...
            current_candidates = self.learned_reranker.rerank(query, current_candidates, top_k=20)
        ...
        if self.llm_reranker and len(current_candidates) > 15:
            ...
            current_candidates = await self.llm_reranker.rerank(query, current_candidates)
        ...
        if self.cross_encoder:
            ...
            final_candidates = await self.cross_encoder.rerank(query, current_candidates, top_k=top_k)
        else:
            final_candidates = current_candidates[:top_k]

        logger.info(f"Hybrid final reranking: {len(candidates)} → {len(final_candidates)} chunks")
