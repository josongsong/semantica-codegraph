...
class MLIntentClassifier:
    ...
    def _extract_features(self, query: str) -> MLIntentFeatures:
        ...
        has_path = bool(re.search(r"[./\\]", query) or any(".py" in t or ".ts" in t for t in tokens))
        ...
        question_words = sum(1 for qw in ["what", "where", "when", "how", "why", "which", "who"] if qw in query.lower())
    ...
    def _predict_with_model(self, features: MLIntentFeatures) -> tuple[IntentKind, float]:
        """
    ...
    def _heuristic_classify(self, query: str) -> tuple[IntentKind, float]:
        ...
        if len(query.split()) > 5 and not any(c in query for c in ["(", ")", "{", "}", "[", "]"]):
