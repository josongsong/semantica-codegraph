...
if TYPE_CHECKING:
    from src.contexts.retrieval_search.infrastructure.factory import RetrieverConfig
...
@retriever_registry.register(
    "optimized",
    description="Optimized retriever with caching and reranking",
    features=[
        "embedding_cache",
        "llm_reranker_cache",
        "dependency_ordering",
...
class OptimizedRetrieverWrapper:
    """Wrapper for OptimizedRetrieverService."""
    ...
    def _create_service(self) -> Any:
        from src.contexts.retrieval_search.infrastructure.service_optimized import RetrieverServiceFactory

        return RetrieverServiceFactory.create_optimized(
            vector_index=self._container.vector_index,
            lexical_index=self._container.lexical_index,
            symbol_index=self._container.symbol_index,
            graph_index=getattr(self._container, "graph_index", None),
            optimization_level=self._config.optimization_level.value,
        )
...
class V3OrchestratorWrapper:
    ...
    def _create_orchestrator(self) -> Any:
        from src.contexts.retrieval_search.infrastructure.v3.config import CrossEncoderConfig, RetrieverV3Config
    ...
    async def retrieve(
        ...
        **kwargs: Any,
        ...
        if self._scope_selector:
            try:
                from src.contexts.retrieval_search.infrastructure.intent.models import QueryIntent
...
class ReasoningRetrieverWrapper:
    ...
    def _create_retriever(self) -> Any:
        from src.contexts.retrieval_search.infrastructure.reasoning.test_time_compute import ReasoningRetriever
