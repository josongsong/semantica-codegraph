...
class ShadowModeRunner:
    ...
    async def run_shadow(
        ...
        try:
            ...
            shadow_result = await asyncio.wait_for(shadow_func(query), timeout=self.shadow_timeout)
            shadow_latency = (time.time() - start_shadow) * 1000
        except asyncio.TimeoutError:
            logger.warning(f"Shadow execution timeout for query: {query[:50]}... (>{self.shadow_timeout}s)")
            shadow_result = None
        ...
        if rank_correlation is not None and rank_correlation < 0.7:
            logger.info(f"Shadow mode: Low rank correlation ({rank_correlation:.3f}) for query: {query[:50]}...")

        return result

    def _calculate_rank_correlation(self, prod_result: Any, shadow_result: Any) -> float | None:
        """
    ...
    def _calculate_hit_at_k(self, prod_result: Any, shadow_result: Any) -> dict[int, bool]:
        """
    ...
    def get_summary_statistics(self) -> dict[str, Any]:
        ...
        correlations = [r.rank_correlation for r in self.shadow_results if r.rank_correlation is not None]
