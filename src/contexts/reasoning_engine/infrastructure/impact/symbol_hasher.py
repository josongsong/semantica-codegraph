...
from src.contexts.code_foundation.domain.models import GraphNode as IRNode
from src.contexts.code_foundation.infrastructure.ir.models import EdgeKind, IRDocument, NodeKind
from src.contexts.reasoning_engine.domain.models import ImpactLevel, SymbolHash
...
class ImpactClassifier:
    ...
    def classify_change(
        ...
        if old_signature_hash != new_signature_hash:
            return ImpactLevel.SIGNATURE_CHANGE

        # 2. Body만 변경 → IR_LOCAL
        if old_body_hash != new_body_hash:
            return ImpactLevel.IR_LOCAL
