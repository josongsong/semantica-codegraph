...
class RiskAnalyzer:
    ...
    def analyze_risk(self, patch: SpeculativePatch, delta_graph: DeltaGraph, base_graph: Any) -> RiskReport:
        ...
        try:
            ...
            raise RiskAnalysisError(f"Analysis failed: {e}")
    ...
    def _compute_affected_symbols(self, patch: SpeculativePatch, delta_graph: DeltaGraph, base_graph: Any) -> set[str]:
        ...
        while worklist:
            ...
            if symbol in visited or current_depth >= max_depth:
                continue
    ...
    def _calculate_risk_score(
        ...
        """
        # Base: affected symbols (normalized to 0-0.7)
        base_score = min(len(affected_symbols) / 100.0, 0.7)

        # Penalty: breaking changes (0-0.3)
        breaking_penalty = min(len(breaking_changes) * 0.15, 0.3)

        # Penalty: patch confidence (low confidence = higher risk)
        confidence_penalty = (1.0 - patch.confidence) * 0.1
