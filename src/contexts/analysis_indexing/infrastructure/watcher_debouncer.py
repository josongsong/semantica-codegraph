...
class EventDebouncer:
    ...
    사용 예:
        ...
        debouncer.push_event(FileEventType.MODIFIED, "src/main.py")
    """
    ...
    async def start(self):
        ...
        logger.info(
            "event_debouncer_started", debounce_ms=self.debounce_ms, max_batch_window_ms=self.max_batch_window_ms
        )
    ...
    def push_event(self, event_type: FileEventType, file_path: str, dest_path: str | None = None):
        ...
        self._events[file_path] = event

        # moved 이벤트: dest_path도 추가 (created로)
        if event_type == FileEventType.MOVED and dest_path:
            self._events[dest_path] = FileEvent(
