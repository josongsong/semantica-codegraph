...
class GraphDiffer:
    ...
    def compare_reachability(self, start_symbol: str) -> SemanticDiff:
        ...
        old_reachable = self._compute_reachable(
            start_symbol,
            self.context.old_call_graph
        )
        new_reachable = self._compute_reachable(
            start_symbol,
            self.context.new_call_graph
        )
    ...
    def _get_callees(self, symbol_id: str, call_graph: Optional[Any]) -> Set[str]:
