...
class SpeculativeExecutor:
    ...
    def execute(self, patch: SpeculativePatch) -> SpeculativeResult:
        ...
        callers_affected, callees_affected = self._analyze_call_graph_impact(
            patch, graph_delta
        )
        
        # Step 6: Generate recommendations
        recommendations = self.risk_analyzer.generate_recommendations(
            patch, graph_delta, risk_level
        )
    ...
    def _find_breaking_changes(
        ...
        if delta.nodes_removed:
            for node in delta.nodes_removed:
                ...
                if self.context.call_graph:
                    ...
                    if callers:
                        breaking.append(
                            f"Removing {node} breaks {len(callers)} caller(s)"
                        )
        
        # Signature changes
        if patch.patch_type.value == "change_signature":
            breaking.append(
                f"Signature change to {patch.target_symbol} may break callers"
            )
        ...
        if patch.patch_type.value == "rename":
            ...
            if ref_count > 5:
                breaking.append(
                    f"Renaming {patch.target_symbol} affects {ref_count} references"
                )
