...
class RebuildMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rebuild_ratio": (
                self.symbols_rebuilt / self.symbols_analyzed
...
class PartialGraphRebuilder:
    ...
    async def rebuild(
        ...
        ir_docs: Optional[Dict[str, Any]] = None,
    ) -> RebuildMetrics:
        """
    ...
    async def _rebuild_parallel(
        ...
        batches = [
            symbol_list[i:i + strategy.batch_size]
            for i in range(0, len(symbol_list), strategy.batch_size)
        ]
        
        for batch in batches:
            tasks = [
                self._rebuild_symbol(symbol_id, strategy)
    ...
    def __repr__(self) -> str:
        return (
            f"PartialGraphRebuilder("
            f"rebuilt={self.metrics.symbols_rebuilt}, "
            f"skipped={self.metrics.symbols_skipped})"
        )
