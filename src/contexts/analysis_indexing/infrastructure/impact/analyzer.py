...
class ImpactAnalyzer:
    ...
    async def analyze_changes(
        ...
        for file_path in all_files:
            ...
            # File modified
                ...
                file_impacts = self._analyze_file_modified(
                    file_path, base_doc, new_doc
                )
    ...
    def _analyze_file_modified(
        ...
        base_nodes = {
            ...
            for node in getattr(base_doc, "nodes", [])
            if getattr(node, "id", "")
        }
        
        new_nodes = {
            getattr(node, "id", ""): node
            for node in getattr(new_doc, "nodes", [])
            if getattr(node, "id", "")
        }
        ...
        for symbol_id in base_nodes.keys() & new_nodes.keys():
            ...
            impact = self._analyze_symbol_change(
                file_path, symbol_id, base_node, new_node
            )
            if impact:
                impacts.append(impact)
    ...
    def _analyze_symbol_change(
        ...
        if base_location and new_location:
            base_lines = (
                getattr(base_location, "end_line", 0) -
                getattr(base_location, "start_line", 0)
            )
            new_lines = (
                getattr(new_location, "end_line", 0) -
                getattr(new_location, "start_line", 0)
            )
    ...
    def _build_strategy(self, result: ImpactAnalysisResult) -> RebuildStrategy:
        ...
        logger.info(
            ...
            components=[
                c for c in ["CFG", "DFG", "CG", "TG"]
                if getattr(strategy, f"rebuild_{c.lower()}", False)
            ],
        )
    ...
    def __repr__(self) -> str:
        return (
            f"ImpactAnalyzer("
            f"base_files={len(self._base_ir)}, "
            f"new_files={len(self._new_ir)})"
        )
