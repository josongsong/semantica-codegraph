...
class RepoMapIdGenerator:
    ...
    def generate(ctx: RepoMapIdContext) -> str:
        ...
        # For very long identifiers, use hash suffix (SHA-256 for better collision resistance)
        if len(identifier) > 200:
            hash_suffix = hashlib.sha256(identifier.encode()).hexdigest()[:12]  # 12 chars = 48 bits
