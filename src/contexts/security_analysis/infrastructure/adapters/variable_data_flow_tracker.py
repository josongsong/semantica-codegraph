...
class VariableDataFlowTracker:
    ...
    def find_data_flow_paths(
        ...
        print(f"[DATA FLOW] Starting path search: {source_id} → {sink_id}")
        ...
        while queue and len(paths) < max_paths:
            ...
            if iterations <= 10:  # Debug first 10 iterations
                print(
                    f"[BFS {iterations}] Current: {self._node_name(current_id, ir_document)}, Depth: {depth}, Path len: {len(path)}"
                )
