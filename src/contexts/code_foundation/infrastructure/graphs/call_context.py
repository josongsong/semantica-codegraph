...
class CallContext:
    ...
    def get_argument(self, param_name: str) -> Any | None:
...
class ContextSensitiveCallGraph:
    ...
    _basic_edges: set[tuple[str, str]] | None = None
    ...
    def get_callees(self, caller: str, context: CallContext | None = None) -> set[str]:
    ...
    def compare_with_basic(self, basic_cg: set[tuple[str, str]]) -> dict[str, Any]:
        ...
        return {
            ...
            "precision_gain_pct": ((len(basic_cg) - len(cs_basic)) / len(basic_cg) * 100 if basic_cg else 0),
        }
