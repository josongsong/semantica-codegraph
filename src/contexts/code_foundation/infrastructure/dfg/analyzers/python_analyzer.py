...
from src.contexts.code_foundation.infrastructure.dfg.statement_analyzer import BaseStatementAnalyzer
...
class PythonStatementAnalyzer(BaseStatementAnalyzer):
    ...
    def _extract_identifiers_smart(self, node: "TSNode") -> list[str]:
        ...
        if node.type in (
            ...
            # Collect all loop variables first
            loop_vars = set()
            for child in node.children:
            ...
            for child in node.children:
                if child.type == "for_in_clause":
                    # Process the for_in_clause specially (reads from iterable)
                    ...
                    identifiers.extend([ident for ident in child_identifiers if ident not in loop_vars])
            return identifiers
    ...
    def _extract_comprehension_loop_vars(self, for_in_clause: "TSNode") -> list[str]:
        """
        Extract loop variables from a for-in clause in comprehension.

        for x in items → loop vars: [x]
        for x, y in items → loop vars: [x, y]

        Args:
            for_in_clause: for_in_clause node

        Returns:
            List of loop variable names
        """
        loop_vars = []
