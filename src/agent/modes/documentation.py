...
class DocumentationMode(BaseModeHandler):
    ...
    def _determine_doc_type(self, task: Task) -> str:
        ...
        if any(kw in query_lower for kw in ["docstring", "function doc", "class doc", "document class", "document function"]):
            return "docstring"
    ...
    async def _generate_docstrings(
        self, targets: list[dict], code: str, context: ModeContext
    ) -> str:
        """
