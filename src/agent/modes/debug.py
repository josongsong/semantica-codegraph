...
from src.agent.utils import get_file_context, read_multiple_files
...
class DebugMode(BaseModeHandler):
    ...
    async def execute(self, task: Task, context: ModeContext) -> Result:
        ...
        try:
            fix_proposal = await self._generate_fix(
                error_info, error_location, related_code, context
            )
    ...
    def _analyze_stacktrace(self, error_info: dict) -> dict | None:
        ...
        if matches:
            ...
            return {
                ...
                "frames": [
                    {"file": m[0], "line": int(m[1]), "function": m[2] or "unknown"}
                    for m in matches
                ],
            }
    ...
    async def _find_local_handlers(self, symbol_id: str) -> list[dict]:
        ...
        try:
            ...
            handlers = self.graph.get_neighbors(
                node_id=symbol_id,
                edge_type="CFG_HANDLER",
                direction="outgoing"
            )
    ...
    async def _find_caller_handlers(self, symbol_id: str, max_depth: int = 3) -> list[dict]:
        ...
        try:
            ...
            for caller in callers[:max_depth]:  # Limit depth
                ...
                has_handler = self.graph.get_neighbors(
                    node_id=caller_id,
                    edge_type="CFG_HANDLER",
                    direction="outgoing"
                )
    ...
    async def _generate_fix(
        ...
        """
        if not self.llm:
            # Fallback: return placeholder fix
            self.logger.warning("No LLM client provided, using placeholder fix")
...
"""
    ...
    def _create_fix_changes(
        self, fix_code: str, error_location: dict | None, context: ModeContext
    ) -> list[Change]:
        """
