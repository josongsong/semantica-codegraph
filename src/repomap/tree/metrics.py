...
from src.repomap.models import RepoMapBuildConfig, RepoMapNode
...
class EntrypointDetector:
    ...
    def _is_entrypoint(node: RepoMapNode) -> bool:
        """Check if node is an entrypoint."""
        # Check filename
        if node.path:
            filename = node.path.lower()
            if any(keyword in filename for keyword in ["main", "cli", "app", "server", "router", "routes", "__main__"]):
                return True

        # Check FQN
        if node.fqn:
            fqn_lower = node.fqn.lower()
            if any(keyword in fqn_lower for keyword in ["main", "route", "endpoint", "handler", "entrypoint"]):
                return True
...
class TestDetector:
    ...
    def detect(nodes: list[RepoMapNode]) -> None:
        ...
        Heuristics:
        - Path contains: tests/, test/, __tests__/
    ...
    def _is_test(node: RepoMapNode) -> bool:
        ...
        if node.path:
            ...
            if any(test_dir in path_lower for test_dir in ["tests/", "test/", "__tests__/", "/tests/"]):
                return True
