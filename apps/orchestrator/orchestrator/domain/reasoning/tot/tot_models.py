...
# ============================================================================
# ToT + Test Result (Domain Model)
# ============================================================================


@dataclass
class ToTWithTestResult:
    """
    ToT + Test 검증 결과 (P0-2)

    ToT로 생성한 전략을 실제 테스트로 검증한 결과
    """
    ...
    # Selected Strategy
    selected_strategy: CodeStrategy | None = None
    ...
    def is_success(self) -> bool:
        """성공 여부"""
        return self.success and self.selected_strategy is not None

    def get_summary(self) -> str:
        """요약 생성"""
        if self.success:
            return (
                f"성공: {self.selected_strategy.title} "
                f"(시도 {self.attempts}/{self.total_strategies}, "
                f"점수 {self.selected_score:.2f})"
            )
        return f"실패: {self.error or 'All strategies failed'} (시도 {self.attempts}/{self.total_strategies})"
