...
from apps.orchestrator.orchestrator.domain.diff_manager import FileDiff
...
class ApprovalSession:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        # Pending 계산: 파일 레벨 결정이 없는 파일들
        decided_files = {d.file_path for d in self.decisions if d.hunk_index is None}
        pending = len([f for f in self.file_diffs if f.file_path not in decided_files])
...
class ApprovalManager:
    ...
    def should_auto_approve(self, file_diff: FileDiff) -> bool:
        ...
        Returns:
            True면 자동 승인
        """
        return self.criteria.should_auto_approve(file_diff)
