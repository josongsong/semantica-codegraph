...
class PartialCommitter:
    ...
    async def apply_partial(
        ...
        try:
            ...
            # 4. Patch 적용 (git apply → fuzzy fallback)
    ...
    async def _apply_patch(self, patch: str, file_path: str | None = None) -> None:
        """
        Patch 적용 (git apply → fuzzy fallback).

        Args:
            patch: Git patch 문자열
            file_path: 대상 파일 경로 (Optional, Fuzzy Patcher용)
            ...
            if result.returncode != 0:
                ...
                if result.returncode != 0:
                    ...
                    if self.fuzzy_patcher and file_path:
                        logger.warning(
                            f"git apply failed for {file_path}, "
                            f"trying CASCADE Fuzzy Patcher... (stderr: {result.stderr})"
                        )
                        ...
                        try:
                            ...
                            if fuzzy_result.is_success():
                                ...
                                # Fuzzy 성공 → 파일이 수정됨 → git add 필요 (apply_hunks에서 처리)
                                return
