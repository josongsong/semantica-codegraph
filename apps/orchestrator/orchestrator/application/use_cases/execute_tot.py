...
from apps.orchestrator.orchestrator.shared.reasoning.tot.tot_models import ToTResult

if TYPE_CHECKING:
    from codegraph_agent.ports.reasoning import IToTExecutor
...
class ExecuteToTUseCase:
    ...
    async def _execute_strategies_parallel(self, strategies: list) -> dict:
        ...
        # Dict로 변환
        results = {}
        for strategy, result in zip(strategies, results_list, strict=False):
