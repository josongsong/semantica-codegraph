...
class LATSSearchEngine:
    ...
    def _save_winning_path(self, winning_path: WinningPath):
        """
        Winning Path를 Session Memory에 저장 (SOTA: contexts/session_memory)

        BEFORE: agent/experience (단순 저장)
        AFTER: contexts/session_memory (완전 학습!)
        """
        if not winning_path:
            return

        # 1. 파일 저장 (JSONL - 호환성 유지)
        ...
        # 2. SOTA: Session Memory 저장 (완전 기능!)
        if self.experience_repo:
            try:
                # SOTA: contexts/session_memory 사용
                from codegraph_runtime.session_memory.domain.models import Episode, TaskStatus, TaskType

                # Convert to Episode (완전 모델)
                episode = Episode(
                    id=f"lats_{timestamp}_{problem_hash}",
                    project_id="default",
                    session_id=f"session_{timestamp}",
                    task_type=self._map_problem_type(winning_path.problem_type),
                    task_description=winning_path.problem_description,
                    task_complexity=0.7 if winning_path.total_iterations > 5 else 0.5,
                    files_involved=list(winning_path.final_code_changes.keys()),
                    symbols_involved=[],
                    error_types=[],
                    stack_trace_signature=None,
                    plan_summary=f"LATS with {winning_path.total_iterations} iterations",
                    steps_count=winning_path.total_iterations,
                    tools_used=["lats", "mcts"],
                    key_decisions=[f"Selected: {winning_path.final_strategy_id}"],
                    pivots=[],
                    outcome_status=TaskStatus.SUCCESS
                    if winning_path.reflection_verdict == "ACCEPT"
                    else TaskStatus.FAILURE,
                    patches=list(winning_path.final_code_changes.values()),
                    tests_passed=winning_path.execution_result.get("test_pass_rate", 0.0) > 0.8,
                    user_feedback=None,
                    problem_pattern=winning_path.problem_type,
                    solution_pattern=winning_path.final_strategy_id,
                    gotchas=[],
                    tips=[],
                    duration_ms=None,
                    tokens_used=None,
                    retrieval_count=0,
                    usefulness_score=winning_path.final_q_value,
                    created_at=datetime.now(),
                )

                # SOTA: Episode 저장 → 자동 학습!
                # - Bug pattern 추출
                # - Code pattern 학습
                # - Project knowledge 업데이트
                if hasattr(self.experience_repo, "consolidate_session"):
                    # contexts/session_memory
                    import asyncio

                    asyncio.create_task(self.experience_repo.consolidate_session(episode))
                    logger.info("episode_saved_to_session_memory", episode_id=episode.id)
                else:
                    # Fallback: agent/experience (기존)
                    from apps.orchestrator.orchestrator.domain.experience import AgentExperience, ProblemType
                    ...
                    logger.info("experience_saved_to_simple_store")

            except Exception as e:
                logger.warning(f"Failed to save experience: {e}")

    def _map_problem_type(self, problem_type: str):
        """Map problem type string to TaskType enum"""
        from codegraph_runtime.session_memory.domain.models import TaskType

        mapping = {
            "bugfix": TaskType.DEBUG,
            "feature": TaskType.IMPLEMENT,
            "refactor": TaskType.REFACTOR,
            "test": TaskType.TEST,
            "review": TaskType.REVIEW,
        }
        return mapping.get(problem_type.lower(), TaskType.UNKNOWN)
